...
local function greetCallback(npc, creature)
	...
	if player then
		npcHandler:setMessage(
			MESSAGE_GREET,
			"Greeting, |PLAYERNAME|! I welcome you to this sacred {place}. \z
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "missions") then
		...
		if player:getStorageValue(Storage.CultsOfTibia.FinalBoss.Mission) > 2 then
			npcHandler:say(
				"You have already fulfilled your job to my full satisfaction. \z
			The cults are investigated and the final boss is eliminated. \z
			I have nothing more for you to do. Fare you well!",
				npc,
				creature
			)
			npcHandler:setTopic(playerId, 0)
		elseif
			player:getStorageValue(Storage.CultsOfTibia.Minotaurs.Mission) == 6
			...
			and player:getStorageValue(Storage.CultsOfTibia.FinalBoss.Mission) < 2
		then
			npcHandler:say(
				"It seems to me that you have done all the missions I gave you. \z
				All the cults have been revealed and now you can kill their leader, the final boss, \z
				to save the world from a possible catastrophe.",
				npc,
				creature
			)
			npcHandler:setTopic(playerId, 0)
		...
		elseif player:getStorageValue(Storage.CultsOfTibia.FinalBoss.Mission) == 2 then
			...
			npcHandler:say(
				"In wich of the following topics are you interested in? Cult of {Life}, \z
				Cult of {Prosperity}, Cult of the {Minotaurs}, Cult of the {Barkless}, Cult of the {Misguided}, \z
				Cult of {Orcs} or Cult of the {Humans}?",
				npc,
				creature
			)
