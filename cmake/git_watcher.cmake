...
function(get_git_state _working_dir)
    ...
    # run_git_command ever returns a non-zero exit code.
    ...
    set(object HEAD)
    run_git_command(show -s "--format=%H" ${object})
    if(exit_code EQUAL 0)
        set(ENV{GIT_HEAD_SHA1} ${output})
    endif()

    run_git_command(show -s "--format=%an" ${object})
    if(exit_code EQUAL 0)
        set(ENV{GIT_AUTHOR_NAME} "${output}")
    endif()

    run_git_command(show -s "--format=%ae" ${object})
    if(exit_code EQUAL 0)
        set(ENV{GIT_AUTHOR_EMAIL} "${output}")
    endif()

    run_git_command(show -s "--format=%cI" ${object})
    if(exit_code EQUAL 0)
        set(ENV{GIT_COMMIT_DATE_ISO8601} "${output}")
    endif()

    run_git_command(show -s "--format=%s" ${object})
    if(exit_code EQUAL 0)
        set(ENV{GIT_COMMIT_SUBJECT} "${output}")
    endif()

    run_git_command(show -s "--format=%b" ${object})
    if(exit_code EQUAL 0)
        set(ENV{GIT_COMMIT_BODY} "${output}")
    endif()

    # run_git_command(rev-parse --short ${object})
    # if(exit_code EQUAL 0)
    #     set(ENV{GIT_SHORT_SHA1} ${output})
    # endif()

    # run_git_command(describe --tags "--match=v*" ${object})
    # if(exit_code EQUAL 0)
...
function(check_git _working_dir _state_changed)
...
function(setup_git_monitoring)
    add_custom_target(
        ...
        COMMAND
            ${CMAKE_COMMAND} -D_BUILD_TIME_CHECK_GIT=TRUE -DGIT_WORKING_DIR=${GIT_WORKING_DIR}
            -DGIT_EXECUTABLE=${GIT_EXECUTABLE} -DGIT_STATE_FILE=${GIT_STATE_FILE}
            -DPRE_CONFIGURE_FILE=${PRE_CONFIGURE_FILE} -DPOST_CONFIGURE_FILE=${POST_CONFIGURE_FILE} -P
