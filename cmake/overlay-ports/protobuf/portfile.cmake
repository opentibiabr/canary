...
if(VCPKG_DOWNLOAD_MODE)
    # download PKGCONFIG in download mode which is used in
    # `vcpkg_fixup_pkgconfig()` at the end of this script. download it here
    # because `vcpkg_cmake_configure()` halts execution in download mode when
    # running configure process.
    vcpkg_find_acquire_program(PKGCONFIG)
endif()

# Delete language backends we aren't targeting to reduce false positives in
# automated dependency detectors like Dependabot.
file(
    REMOVE_RECURSE
    "${SOURCE_PATH}/csharp"
    "${SOURCE_PATH}/java"
...
if(protobuf_BUILD_PROTOC_BINARIES)
    if(VCPKG_TARGET_IS_WINDOWS)
        ...
        vcpkg_copy_tools(
            TOOL_NAMES
            protoc
            protoc-${VERSION_MINOR}.${VERSION_PATCH}.0
            AUTO_CLEAN
        )
    endif()
else()
    file(
        COPY "${CURRENT_HOST_INSTALLED_DIR}/tools/${PORT}"
        DESTINATION "${CURRENT_PACKAGES_DIR}/tools"
    )
endif()

vcpkg_replace_string(
    "${CURRENT_PACKAGES_DIR}/share/${PORT}/protobuf-config.cmake"
    "if(protobuf_MODULE_COMPATIBLE)"
    "if(protobuf_MODULE_COMPATIBLE OR CMAKE_FIND_PACKAGE_NAME STREQUAL \"Protobuf\")"
)
if(NOT protobuf_BUILD_LIBPROTOC)
    vcpkg_replace_string(
        "${CURRENT_PACKAGES_DIR}/share/${PORT}/protobuf-module.cmake"
...
function(replace_package_string package)
    set(debug_file
        "${CURRENT_PACKAGES_DIR}/debug/lib/pkgconfig/${package}.pc"
    )
    set(release_file
        "${CURRENT_PACKAGES_DIR}/lib/pkgconfig/${package}.pc"
    )
...
if(NOT protobuf_BUILD_PROTOC_BINARIES)
    configure_file(
        "${CMAKE_CURRENT_LIST_DIR}/protobuf-targets-vcpkg-protoc.cmake"
        "${CURRENT_PACKAGES_DIR}/share/${PORT}/protobuf-targets-vcpkg-protoc.cmake"
        COPYONLY
    )
endif()
