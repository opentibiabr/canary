...
mType.onThink = function(monster, interval)
	...
	elseif monsterPosition and TwentyYearsACookQuest.TheRestOfRatha.BossZone:isInZone(monsterPosition) then
		...
		if conditionsTicks > 0 then
			local tile = Tile(monsterPosition)
			if conditionsTicks % 2000 == 0 then
				monsterPosition:sendMagicEffect(CONST_ME_BLACK_BLOOD)
			end
			if tile and not tile:getItemById(TwentyYearsACookQuest.TheRestOfRatha.Items.GhostItem) then
				effectSent = false
				local item = Game.createItem(TwentyYearsACookQuest.TheRestOfRatha.Items.GhostItem, 1, monsterPosition)
				...
				end, 6000, monsterPosition)
			end
		elseif not effectSent then
			effectSent = true
			monsterPosition:sendMagicEffect(CONST_ME_MORTAREA)
		end
	end
end
