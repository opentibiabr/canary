...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif MsgContains(message, "assistance") then
		if npcHandler:getTopic(playerId) == 2 then
			npcHandler:say({ "Wretched creatures of ice have stolen my egg that was close to hatching. ...", " Since I'm to huge to enter those lower Tunnels I have to ask you to take care of my {egg}. Will you do this?" }, npc, creature)
	...
	if MsgContains(message, "egg") then
		if npcHandler:getTopic(playerId) == 4 then
			npcHandler:say({ "As I told you, fiendish ice creatures dragged my egg into the lower caves. ...", " Without enough heat the egg will die soon. Venture there and save my hatchling and the knowledge you seeek shall be yours!" }, npc, creature)
			player:setStorageValue(Storage.ForgottenKnowledge.BabyDragon, 1)
		end
	end
	return true
