...
keywordHandler:addKeyword({ "name" }, StdModule.say, {
	...
	text = " My name is now known only to the wind and it shall remain like this until I will return to my kin."
})
keywordHandler:addKeyword({ "job" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "I was a guardian of this glade. I am the last one... everyone had to leave."
})
keywordHandler:addKeyword({ "time" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "This glade's time is growing short if nothing will be done soon."
})
keywordHandler:addKeyword({ "forest fury" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "Take care, guardian."
})
keywordHandler:addKeyword({ "orclops" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "Cruel beings. Large and monstrous, with a single eye, staring at their prey. "
})
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "cages") and player:getStorageValue(Storage.ForgottenKnowledge.BirdCage) == 1 then
		npcHandler:say(
			{ "Crafted generations ago to rob a creature of its freedom for that it may earn it again truthfully. You will need them if you plan on returning the birds to their rightful home in the glade. ... ", "Are you in need of another one? " },
			npc,
			creature
		)
