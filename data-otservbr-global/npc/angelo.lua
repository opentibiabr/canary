...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif npcHandler:getTopic(playerId) == 2 and MsgContains(message, "cave") then
		npcHandler:say(
			{
				"We don't know exactly why this cave has now exposed an entry via the {dark pyramid}. It seems that the cave already existed for a long time, however, without a connection to our world. Maybe some smaller earth movements have changed the situation."
			},
			npc,
			creature
		)
	...
	if player:getStorageValue(Storage.CultsOfTibia.Life.Mission) == 3 then
		if MsgContains(message, "mission") then
			npcHandler:say(
				{ "The scientists are still missing? You just found some strange green shining mummies and a big oasis? I give you this analysis tool for the water of the oasis. Maybe that's the key. Could you bring me a sample of this water?" },
				npc,
				creature
			)
	...
	if player:getStorageValue(Storage.CultsOfTibia.MotA.Mission) == 6 then
		if MsgContains(message, "magnifier") then
			npcHandler:say(
				{ "{Gareth} told you that there are rumours about fake artefacts in the MoTA? And it is your task to check that with a magnifier? I see. I don't need one right now, so you can have one of mine. You find one in the crate over there." },
				npc,
				creature
			)
