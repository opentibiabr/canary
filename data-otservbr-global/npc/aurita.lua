...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		...
		elseif player:getStorageValue(ThreatenedDreams.Mission03[1]) == 1 then
			npcHandler:say({ "There is a fairy who once told me about this spell. Perhaps she will share her knowledge. You can find her in a small fairy village in the southwest of Feyrist." }, npc, creature)
		...
		elseif player:getStorageValue(ThreatenedDreams.Mission03[1]) >= 2 and player:getStorageValue(ThreatenedDreams.Mission03[1]) <= 3 then
			npcHandler:say({ "The spell works! I already took a walk on the beach last night in the moonlight. Please talk to Taegen now, he may have another request." }, npc, creature)
			npcHandler:setTopic(playerId, 0)
		else
			npcHandler:say({ "Thank you again, friend! Taegen and I we're so happy now." }, npc, creature)
			npcHandler:setTopic(playerId, 0)
		end
	elseif MsgContains(message, "starlight vial") or MsgContains(message, "empty starlight vial") then
		if player:getStorageValue(ThreatenedDreams.Mission03[1]) == 4 then
			if player:getStorageValue(ThreatenedDreams.Mission03.EmptyStarlightVial) < 1 then
				...
				npcHandler:say({ "I already gave you the empty starlight vial." }, npc, creature)
			end
		end
