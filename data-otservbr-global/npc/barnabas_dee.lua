...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") and player:getStorageValue(Storage.Oramond.PeppermoonBell) < 1 and player:getStorageValue(Storage.Quest.U10_50.DarkTrails.Mission16) < 1 then
		npcHandler:say({
		...
		npcHandler:setTopic(playerId, 0)
	elseif MsgContains(message, "seance") and player:getStorageValue(Storage.Oramond.PeppermoonBell) == 1 and player:getStorageValue(Storage.Quest.U10_50.DarkTrails.Mission16) < 1 then
		npcHandler:say("Ah! Did you bring me the peppermoon bell pollen I asked for?", npc, creature)
		npcHandler:setTopic(playerId, 1)
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			if player:getStorageValue(Storage.Oramond.PeppermoonBellCount) >= 15 and player:getItemCount(21089) >= 15 then
