...
local function addBuyableKeyword(keywords, itemid, amount, price, text)
	...
	if type(keywords) == "table" then
		keyword = keywordHandler:addKeyword({ "goshak", keywords[1], keywords[2] }, StdModule.say, {
			npcHandler = npcHandler,
			text = text
		})
	else
		keyword = keywordHandler:addKeyword({ "goshak", keywords }, StdModule.say, {
			npcHandler = npcHandler,
			text = text
		})
	end

	keyword:addChildKeyword(
		{ "mok" },
		StdModule.say,
		{
			npcHandler = npcHandler,
			text = "Maruk rambo zambo!",
			reset = true
		},
		function(player)
	...
	keyword:addChildKeyword({ "mok" }, StdModule.say, {
		npcHandler = npcHandler,
		text = "Maruk nixda!",
		reset = true
	})
	keyword:addChildKeyword({ "" }, StdModule.say, {
		npcHandler = npcHandler,
		text = "Buta maruk klamuk!",
		reset = true
	})
end
...
keywordHandler:addKeyword({ "ikem", "goshak" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "Ikem pashak porak, bata, dora. Ba goshak maruk?"
})
keywordHandler:addKeyword({ "goshak", "porak" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "Ikem pashak charcha, burka, burka bata, hakhak. Ba goshak maruk?"
})
keywordHandler:addKeyword({ "goshak", "bata" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "Ikem pashak aka bora, tulak bora, grofa. Ba goshak maruk?"
})
keywordHandler:addKeyword({ "goshak", "dora" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "Ikem pashak donga. Ba goshak maruk?"
})
