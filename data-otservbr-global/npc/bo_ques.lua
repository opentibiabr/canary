...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "recipe") or MsgContains(message, "mission") then
		if missionProgress < 1 then
			npcHandler:say({ "My collection of recipes is almost complete. There are only but a few that are missing. ...", "Hmmm... now that we talk about it. There is something you could help me with. Are you interested?" }, npc, creature)
	...
	elseif MsgContains(message, "cookbook") then
		if missionProgress == -1 then
			npcHandler:say({ "I'm preparing the food for all djinns in Ashta'daramai. ...", "Therefore, I'm what is commonly called a cook, although I do not like that word too much. It is vulgar. I prefer to call myself 'chef'." }, npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 1 then
		if MsgContains(message, "yes") then
			npcHandler:say({ "Fine! Even though I know so many recipes, I'm looking for the description of some dwarven meals. ...", "So, if you could bring me a cookbook of the dwarven kitchen, I'll reward you well." }, npc, creature)
...
keywordHandler:addGreetKeyword(
	{ "djanni'hah" },
	{
		...
		text = "Hey! A human! What are you doing in my kitchen, |PLAYERNAME|?"
	}
)
