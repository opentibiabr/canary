	...
	if condition then
		keywordHandler:addKeyword(
			...
			StdModule.say,
			{
				...
				text = "No, no, no, you even are no barb....barba...er.. one of us!!!! Talk to the Jarl first!"
			},
			condition
		)
	end
	if ringCheck then
		local ring = keywordHandler:addKeyword(
			{ keyword },
			StdModule.say,
			{
				npcHandler = npcHandler,
				text = "Ohh, you got a nice ring there! Ya don't have to pay if you gimme the ring and I promise you I will bring you to the correct spot!*HICKS* Alright?"
			},
			ringCheck
		)
		ring:addChildKeyword(
			{ "yes" },
			StdModule.travel,
			{
				npcHandler = npcHandler,
		...
		local normalTravel = ring:addChildKeyword({ "no" }, StdModule.say, {
			npcHandler = npcHandler,
			text = "Give me 50 gold and I bring you to " .. keyword .. ". 'kay?"
		})
		normalTravel:addChildKeyword({ "no" }, StdModule.say, {
			npcHandler = npcHandler,
			text = "You shouldn't miss the experience.",
			reset = true
		})
		if randomNumber then
			normalTravel:addChildKeyword(
				{ "yes" },
				StdModule.travel,
				{
					npcHandler = npcHandler,
...
addTravelKeyword(
	"svargrond",
	"You know a town nicer than this? NICER DICER! Apropos, don't play dice when you are drunk ...",
	Position(32255, 31197, 7),
	function()
...
addTravelKeyword(
	"okolnir",
	"It's nice there. Except of the ice dragons which are not very companionable.",
	Position(32225, 31381, 7),
	function()
		return randomDestination[math.random(#randomDestination)]
	end,
	function()
...
addTravelKeyword(
	"tyrsung",
	"*HICKS* Big, big island east of here. Venorian hunters settled there ..... I could bring you north of their camp.",
	Position(32333, 31227, 7),
	function()
		return randomDestination[math.random(#randomDestination)]
	end,
	function()
...
addTravelKeyword(
	"camp",
	"Both of you look like you could defend yourself! If you want to go there, ask me for a passage.",
	Position(32021, 31294, 7),
	function()
		return randomDestination[math.random(#randomDestination)]
	end,
	function()
...
keywordHandler:addKeyword({ "kick" }, StdModule.kick, {
	...
	destination = { Position(32468, 31176, 7) }
})
