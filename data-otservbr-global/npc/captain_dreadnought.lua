...
local function AddItemPlayer(playerId)
	local player = Player(playerId)
	if not player then return true end
    local inbox = player:getSlotItem(CONST_SLOT_STORE_INBOX)
    if inbox and inbox:getEmptySlots() > 1 then
        
        if (player:getFreeCapacity() / 100) < 10 then
            player:sendTextMessage(MESSAGE_EVENT_ADVANCE,'Voc? precisa de 10 de cap para pegar a recompensa.')
            return false
        end
        ...
        if player:isSorcerer() and player:getStorageValue(98989) ~= 1 then
            exercise = inbox:addItem(28557, 3000)

			player:removeItem(3355, 1)
			player:removeItem(3361, 1)
			player:removeItem(3559, 1)
			player:removeItem(3286, 1)
			player:addItem(2854, 1)
				--Cogumelo
				player:addItem(3725, 50)

                player:addItem(3043, 3)

                player:addItem(3059,1)
                player:addItem(3074,1)
                player:addItem(7991,1)
                player:addItem(7992,1)
                player:addItem(3362,1)
                player:addItem(3552,1)
                player:addItem(3572,1)
                player:setStorageValue(98989, 1)
        		player:say('Você ganhou uma arma de treino. Veja seu Inbox', TALKTYPE_MONSTER_SAY)
				
        elseif player:isDruid() and player:getStorageValue(98989) ~= 1 then
            exercise = inbox:addItem(28556, 3000)
        ...
        elseif player:isPaladin() and player:getStorageValue(98989) ~= 1 then
            exercise = inbox:addItem(28555, 3000)
            ...
            player:addItem(3425,1)
            player:addItem(3277,1)
            player:addItem(3571,1)
            player:addItem(8095,1)
            player:addItem(3552,1)
            player:addItem(3572,1)
        ...
        elseif player:isKnight() and player:getStorageValue(98989) ~= 1 then
			
			exercise_1 = inbox:addItem(35279, 3000)
			exercise_2 = inbox:addItem(35280, 3000)
			exercise_3 = inbox:addItem(35281, 3000)
            ...
            player:addItem(3425,1)
            player:addItem(7773,1)
            player:addItem(3327,1)
            player:addItem(7774,1)
        ...
        if exercise then
            exercise:setActionId(100)
		end
		if exercise_1 then
            exercise_1:setActionId(100)
		end
		if exercise_2 then
            exercise_2:setActionId(100)
		end
		if exercise_3 then
            exercise_3:setActionId(100)
		end
			
    else
        player:sendCancelMessage("Confirme se voc? possui espa o no seu Store Inbox.")
    end
end
...
{
	npcHandler = npcHandler,
	text = "Ent?o voc? est? preparado para ir para a cidade principal? {yes}"
})
...
local defaultTownNode = readyNode:addChildKeyword({"yes"}, StdModule.say,
{
	npcHandler = npcHandler,
	text = {
		"Eu te recomendaria ir para Thais, posso te teletransportar para la? {yes}"
	}
})
...
-- Towns topic nodes
local townsNode = keywordHandler:addKeyword({""}, StdModule.say,
{
	npcHandler = npcHandler,
	text = ""
})
...
-- Sail topic nodes
local sailNode = keywordHandler:addKeyword({"viajar"}, StdModule.say,
{
	npcHandler = npcHandler,
	text = "Qual seria a cidade que voc? quer tornar-se um cidad?o? "  .. townNames.all .. "?"
})
