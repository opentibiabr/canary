...
local towns = {
	[TOWNS_LIST.AB_DENDRIEL] = {
		name = "Ab'dendriel",
		about = { "Main city of the elves - lots of trees, bug milk and stuff that easily burns ...", "... Sorry, just thinking aloud. Ahem. Very cosy and safe I guess if you're fond of nature." },
		canBeSailed = true,
		isPremium = false,
	...
	[TOWNS_LIST.CARLIN] = {
		name = "Carlin",
		about = { "A city ruled by forthright independent women. \z
	...
	[TOWNS_LIST.DARASHIA] = {
		name = "Darashia",
		about = { "One of the two desert cities. \z
	...
	[TOWNS_LIST.EDRON] = {
		name = "Edron",
		about = { "Quiet little castle city on an island in the north-eastern part of Tibia. \z
	...
	[TOWNS_LIST.LIBERTY_BAY] = {
		name = "Liberty Bay",
		about = { "Liberty Bay is on an island group in the South Seas. Ah, home sweet home. Err. I mean, \z
	...
	[TOWNS_LIST.PORT_HOPE] = {
		name = "Port Hope",
		about = { "Port Hope is an outpost right in the middle of the jungle. ...", "Apes, bananas, hydras, tarantulas... Who'd want to go there? \z
	...
	[TOWNS_LIST.SVARGROND] = {
		name = "Svargrond",
		about = { "Negative, can't bring you there. You gotta pass some sort of Barbarian test \z
			before they let you live there. Still, you should go there sometime, I heard it's quite interesting." },
		canBeSailed = false
	},
	[TOWNS_LIST.THAIS] = {
		name = "Thais",
		about = { "Old-school city. Actually the oldest main city in Tibia. \z
	...
	[TOWNS_LIST.VENORE] = {
		name = "Venore",
		about = { "Hohoh, one of the richest cities, filled with merchants and LOOT! Err. \z
	...
	[TOWNS_LIST.YALAHAR] = {
		name = "Yalahar",
		about = { "Now that must be one of the biggest cities I've ever seen. \z
...
local function donationHandler(npc, creature, message, keywords, parameters, node)
	...
	elseif (parameters.confirm == true) then
		if player:getMoney() > npcConfig.moneyToNeedDonation then
			...
			npcHandler:say("Well, that's really generous of you. That'll feed a lot of hungry mouths for a while. \z
...
local function townTravelHandler(npc, creature, message, keywords, parameters, node)
	...
	if (parameters.confirm ~= true) and (parameters.decline ~= true) and parameters.townId then
		...
		elseif town.isPremium == true and not player:isPremium() then
			npcHandler:say("Negative, can't bring you there without a premium account. \z
	...
	elseif (parameters.confirm == true) then
		...
		npcHandler:say("Cast off! Don't forget to talk to the guide at the port for directions to nearest bars... err, shops and \z
...
keywordHandler:addKeyword({ "adventurers guild" }, StdModule.say, {
	npcHandler = npcHandler,
	text = { "Those fellows help still green adventurers like you, so you learn the lay of the Tibian Mainlands. \z
...
-- hi, yes, yes, ...
defaultTownNode:addChildKeyword({ "yes" }, townTravelHandler, {
	confirm = true,
	townId = defaultTown
})
...
for id, town in pairs(towns) do
	local townNode = KeywordNode:new({ town.name:lower() }, StdModule.say, {
		npcHandler = npcHandler,
		text = town.about
	})
