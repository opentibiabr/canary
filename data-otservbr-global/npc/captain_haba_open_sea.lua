...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif npcHandler:getTopic(playerId) == 1 and message:lower() ~= "starboard" then
		npcHandler:say(
			{ "Ya gotta learn a lot! The correct command would be STARBOARD side. ...", "After you told me about the direction, put a bait on the crane again and go up to the lookout! That would be all sailor, let's go hunt down the sea serpent!!" },
			npc,
			creature
		)
...
keywordHandler:addKeyword({ "caves" }, StdModule.say, {
	...
	text = "I'd go by myself and tear that beast's heart out if I were younger. I hope ya can do that for me. Now go and good luck to ya!"
})
keywordHandler:addKeyword({ "wares" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "Ya're a coward, aren't ya? Prove that I'm wrong and get your lazy bones up to the lookout!!"
})
keywordHandler:addAliasKeyword({ "go" })
keywordHandler:addKeyword({ "bait" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "Just ask me for a trade if you need a bait."
})
keywordHandler:addKeyword({ "test" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "I can give ya a challenge to test if ya have the guts to be a real sailor. Go up to the lookout and remain there for 24 hours during a storm! Harharhar!"
})
