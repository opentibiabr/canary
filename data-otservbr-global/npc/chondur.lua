...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "stampor") or MsgContains(message, "mount") then
		if not player:hasMount(11) then
			npcHandler:say("You did bring all the items I requqested, cuild. Good. \
				Shall I travel to the spirit realm and try finding a stampor compasion for you?", npc, creature)
	...
	elseif MsgContains(message, "mission") then
		if player:getStorageValue(Storage.TheShatteredIsles.ReputationInSabrehaven) == 8 then
			npcHandler:say("The evil cult has placed a curse on one of the captains here. \
				I need at least five of their pirate voodoo dolls to lift that curse.", npc, creature)
...
local function addTaskKeyword(text, value, missionStorage)
	local taskKeyword = keywordHandler:addKeyword(
		{ "task" },
		StdModule.say,
		{
			npcHandler = npcHandler,
	...
	local yesKeyword = taskKeyword:addChildKeyword({ "yes" }, StdModule.say, {
		npcHandler = npcHandler,
		text = text[2]
	})
	...
	yesKeyword:addChildKeyword({ "no" }, StdModule.say, {
		npcHandler = npcHandler,
		text = "Would you like me to repeat the task requirements then?",
		moveup = 2
	})

	taskKeyword:addChildKeyword({ "no" }, StdModule.say, {
		npcHandler = npcHandler,
		text = "Well, it seems you aren't ready yet.",
		reset = true
	})
...
local function addItemKeyword(keyword, aliasKeyword, text, value, item, addonId, missionStorage, achievement)
	local itemKeyword = keywordHandler:addKeyword(
		{ keyword },
		StdModule.say,
		{
			npcHandler = npcHandler,
	...
	itemKeyword:addChildKeyword({ "no" }, StdModule.say, {
		npcHandler = npcHandler,
		text = "Well, it seems you aren't ready yet.",
		reset = true
	})
	...
	keywordHandler:addKeyword({ keyword }, StdModule.say, {
		npcHandler = npcHandler,
		text = aliasKeyword and text[4] or text[3]
	})
end
...
addTaskStatusKeyword("addon", "The time has come, my child. I sense great spiritual wisdom in you and I shall grant you a \
	sign of your progress if you can fulfil my task.", 1)
...
local stakeKeyword = keywordHandler:addKeyword(
	...
	{
		npcHandler = npcHandler,
		text = "Would you like to receive a spiritual prayer to bless your stake?"
	},
	function(player)
...
stakeKeyword:addChildKeyword(
	...
	{
		npcHandler = npcHandler,
		text = "You don't have a wooden stake.",
		reset = true
	},
	function(player)
...
stakeKeyword:addChildKeyword(
	...
	{
		npcHandler = npcHandler,
		text = "<mumblemumble> Sha Kesh Mar!",
		reset = true
	},
	function(player)
		return player:getItemCount(5941) > 0
...
keywordHandler:addKeyword(
	...
	{
		npcHandler = npcHandler,
		text = "You should not talk about things you don't know anything about."
	},
	function(player)
...
-- Start mission
local startcounterspellKeyword = keywordHandler:addKeyword(
	{ "counterspell" },
	StdModule.say,
	{
		npcHandler = npcHandler,
...
acceptKeyword:addChildKeyword({ "no" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "It's much safer for you to stay here anyway, trust me.",
	reset = true
})

startcounterspellKeyword:addChildKeyword({ "no" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "It's much safer for you to stay here anyway, trust me.",
	reset = true
})
...
local function addCounterspellKeyword(text, value, itemId)
	local counterspellKeyword = keywordHandler:addKeyword(
		{ "counterspell" },
		StdModule.say,
		{
			npcHandler = npcHandler,
...
addCounterspellKeyword({ "Did you bring the fresh dead chicken?", "Very good! <mumblemumble> 'Your soul shall be protected!' Now, I need a fresh dead rat." }, 1, 4330)
addCounterspellKeyword({ "Did you bring the fresh dead rat?", "Very good! <chants and dances> 'You shall face black magic without fear!' Now, I need a fresh dead black sheep." }, 2, 3994)
addCounterspellKeyword({ "Did you bring the fresh dead black sheep?", "Very good! <stomps staff on ground> 'EVIL POWERS SHALL NOT KEEP YOU ANYMORE! SO BE IT!'" }, 3, 4095)
...
keywordHandler:addKeyword(
	...
	{
		npcHandler = npcHandler,
		text = "Ah, thank you very much! I'll honour his memory."
	},
	function(player)
