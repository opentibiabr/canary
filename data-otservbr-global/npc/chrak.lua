...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "battle") then
		if player:getStorageValue(TheNewFrontier.Questline) == 24 then
			npcHandler:say({ "Zo you want to enter ze arena, you know ze rulez and zat zere will be no ozer option zan deaz or victory?" }, npc, creature)
			npcHandler:setTopic(playerId, 1)
		end
	elseif MsgContains(message, "mission") then
		if player:getStorageValue(TheNewFrontier.Questline) == 24 then
			npcHandler:say({ "Ze tournament iz ze ultimate challenge of might and prowrezz. Ze rulez may have changed over ze centuriez but ze ezzence remained ze zame. ...", "If you know ze rulez, you might enter ze arena for ze {battle}." }, npc, creature)
			npcHandler:setTopic(playerId, 0) --Questlog, "Mission 09: Mortal Combat" --Questlog, "Mission 10: New Horizons"
		elseif player:getStorageValue(TheNewFrontier.Questline) == 27 then
			...
			player:setStorageValue(TheNewFrontier.Mission09[1], 3)
			player:setStorageValue(TheNewFrontier.Mission10[1], 1)
