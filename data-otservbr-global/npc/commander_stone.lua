...
local function creatureSayCallback(npc, creature, type, message)
	...
	if (MsgContains(message, "mission") and player:getStorageValue(Storage.BigfootBurden.QuestLineComplete) >= 2) then
		if player:getStorageValue(Storage.BigfootBurden.Rank) < 30 then
			npcHandler:say(
				{
					"Two missions are available for your {rank}: crystal {keeper} and {spark} hunting. You can undertake each mission but you can turn in a specific mission only once each 20 hours. ...",
		...
		if player:getStorageValue(Storage.BigfootBurden.Rank) < 30 then
			...
			elseif player:getStorageValue(Storage.BigfootBurden.MissionCrystalKeeper) > 0 then -- reporting mission
				if player:getStorageValue(Storage.BigfootBurden.RepairedCrystalCount) >= 5 then -- can report missions
					...
					player:setStorageValue(Storage.BigfootBurden.CrystalKeeperTimout, os.time() + configManager.getNumber(configKeys.BOSS_DEFAULT_TIME_TO_FIGHT_AGAIN))
					...
					npcHandler:setTopic(playerId, 0) -- haven't finished -- is reporting -- se nao tiver reportando
				else
					if npcHandler:getTopic(playerId) >= 1 then
						npcHandler:say("You are not done yet.", npc, creature)
					else
						npcHandler:say("You already have accepted this mission. Don't forget to {report} to me when you are done.", npc, creature)
					end
					npcHandler:setTopic(playerId, 0)
				end
			end
		...
		if player:getStorageValue(Storage.BigfootBurden.Rank) < 30 then
			...
			elseif player:getStorageValue(Storage.BigfootBurden.MissionRaidersOfTheLostSpark) > 0 then -- reporting mission
				if player:getStorageValue(Storage.BigfootBurden.ExtractedCount) >= 7 then -- can report missions
					...
					player:setStorageValue(Storage.BigfootBurden.RaidersOfTheLostSparkTimeout, os.time() + configManager.getNumber(configKeys.BOSS_DEFAULT_TIME_TO_FIGHT_AGAIN))
					...
					if npcHandler:getTopic(playerId) >= 1 then
						npcHandler:say("You did not draw enough energy from Crystal Crushers or you have not asked for this task.", npc, creature)
					else
						npcHandler:say("You already have accepted this mission. Don't forget to {report} to me when you are done.", npc, creature)
					end
					npcHandler:setTopic(playerId, 0)
		...
		if player:getStorageValue(Storage.BigfootBurden.Rank) >= 30 then
			...
			elseif player:getStorageValue(Storage.BigfootBurden.MissionExterminators) > 0 then -- reporting mission
				if player:getStorageValue(Storage.BigfootBurden.ExterminatedCount) >= 10 then -- can report missions
					...
					player:setStorageValue(Storage.BigfootBurden.ExterminatorsTimeout, os.time() + configManager.getNumber(configKeys.BOSS_DEFAULT_TIME_TO_FIGHT_AGAIN))
		...
		if player:getStorageValue(Storage.BigfootBurden.Rank) >= 30 then
			...
			elseif player:getStorageValue(Storage.BigfootBurden.MissionMushroomDigger) > 0 then -- reporting mission
				if player:getStorageValue(Storage.BigfootBurden.MushroomCount) >= 3 then -- can report missions
					...
					player:setStorageValue(Storage.BigfootBurden.MushroomDiggerTimeout, os.time() + configManager.getNumber(configKeys.BOSS_DEFAULT_TIME_TO_FIGHT_AGAIN))
