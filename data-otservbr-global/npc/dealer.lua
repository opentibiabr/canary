...
local config = {
	bet = {
		min = 750, -- gold coins // 30k
		max = 10000000000,
		win = 185, -- 170% high/low
		winNum = 500, -- 300% numbers
	},
	playerPosition = Position(1571, 1343, 6), -- NpcPos(x-2) player must stay on this position to talk with npc
	dicerCounter = Position(1571, 1342, 6), --	NpcPos(x-1, y-1) 	counter position
	diePos = Position(1570, 1342, 6), --NpcPos(y-1)
}
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if table.contains({ "low", "high", "h", "l", "1", "2", "3", "4", "5", "6", "odd", "impar", "par", "even" }, message) then
		...
		if haveDie then
			...
			local item = Game.createItem((5791 + number), 1, config.diePos)
			if item then
