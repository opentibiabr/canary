...
local function creatureSayCallback(npc, creature, type, message)
	...
	if table.contains({ "outfit", "addon" }, message) and player:getStorageValue(Storage.OutfitQuest.PirateBaseOutfit) == 1 then
		npcHandler:say("You're talking about my sabre? Well, even though you earned our trust, \
			you'd have to fulfill a task first before you are granted to wear such a sabre.", creature)
	elseif MsgContains(message, "mission") then
		...
		elseif player:getStorageValue(Storage.TheShatteredIsles.RaysMission2) > 0 and player:getStorageValue(Storage.TheShatteredIsles.TortoiseEggNargorDoor) < 0 then
			npcHandler:say("You did some impressive things. I think people here start considering you as one of us. \
				But these are dire times and everyone of us is expected to give his best and even exceed himself. \
				Do you think you can handle that?", creature)
	...
	elseif MsgContains(message, "task") then
		if player:getStorageValue(storage) < 1 then
			npcHandler:say("Are you up to the task which I'm going to give you and willing to prove you're worthy of wearing such a sabre?", creature)
