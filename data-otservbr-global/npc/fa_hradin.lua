	...
	elseif npcHandler:getTopic(playerId) == 1 then
		if MsgContains(message, "yes") then
			if player:getStorageValue(Storage.DjinnWar.MaridFaction.RataMari) ~= 2 or not player:removeItem(3232, 1) then
				npcHandler:say({ "Don't waste any more time. We need the spyreport of our man in Mal'ouquah as soon as possible! ...", "Also don't forget the password to contact our man: PIEDPIPER!" }, npc, creature)
			else
				npcHandler:say(
					{
...
keywordHandler:addGreetKeyword(
	{ "djanni'hah" },
	{
		...
		text = "Aaaah... what have we here. A human - interesting. And such an ugly specimen, too... All right, human |PLAYERNAME|. How can I help you?"
	}
)
