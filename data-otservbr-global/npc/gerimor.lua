...
keywordHandler:addKeyword({ "persons" }, StdModule.say, {
	npcHandler = npcHandler,
	text = { "Well, while I focus more on the matters of life, some of my peers have different approaches \z
				and emphasize other aspects of the world more in their observations. ...", "Regardless we share a common goal of balance and harmony." }
})
...
local config = {
	missions = {
		...
		barkless = {
			text = { "However, recently they became more prominent as their leader seems to hava taken a turn for the worse. \z
					Rumors of violent acts and disappearing people are linked to this cult. ...", "Someone... Should look into that, don't you think?" },
			completeText = { "Then you got it. Thank you. Here is your reward." },
		...
		orcs = {
			text = { "Several Edron orcs have taken to a dangerous idol it seems. \z
			...
			completeText = { "Then you got it. Thank you. Here is your reward.", "Are you prepared to investigate?" },
			storage = Storage.CultsOfTibia.Orcs.Mission,
			value = 2,
			rewardExp = 25000
		},
		life = {
			text = { "Its worshippers wish for eternal, life free of pain and sorrow. \z
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "missions") then
		...
		elseif player:getStorageValue(Storage.CultsOfTibia.Minotaurs.Mission) == 6 and player:getStorageValue(Storage.CultsOfTibia.Life.Mission) == 10 and player:getStorageValue(Storage.CultsOfTibia.MotA.Mission) == 15 and player:getStorageValue(
			Storage.CultsOfTibia.Barkless.Mission
		) == 7 and player:getStorageValue(Storage.CultsOfTibia.Misguided.Mission) == 5 and player:getStorageValue(Storage.CultsOfTibia.Orcs.Mission) == 3 and player:getStorageValue(Storage.CultsOfTibia.Humans.Mission) == 3 and player:getStorageValue(
			Storage.CultsOfTibia.FinalBoss.Mission
		) < 2 then
