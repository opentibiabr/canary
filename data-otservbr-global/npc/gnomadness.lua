...
local function creatureSayCallback (npc, creature, type, message)
    ...
    if MsgContains(message, "hazard") then
        local messageText = "I can change your hazard level to spice up your hunt in the gardens. Your current level is set to "
            .. current
            .. ". You can select up to {"
            .. selectableMaximum
            .. "}. Your maximum unlocked level so far is {"
            .. maximumUnlocked
            .. "}."
        ...
        if npcHandler:getTopic(playerId) == 1 then
            ...
            if hazard:setPlayerCurrentLevel(player, desiredLevel) then
                ...
                if desiredLevel >= 6 and not player:kv():scoped("primal-ordeal"):get("received-prize") then
                    ...
                    npcHandler:say(
                        "You've achieved the necessary hazard level. As a reward, you've received the Noxious Ripptor mount and a primal bag.",
                        npc,
                        creature
                    )
                end
