...
local function creatureSayCallback(npc, creature, type, message)
	...
	if talkState[playerId] == "delivery" then
		if MsgContains(message, "yes") then
			...
			npcHandler:say(
				{
					"Gnometastic! Here are the parcels. Regrettably, the labels got lost during transport; but I guess those lonely gnomes won't mind as long as they get ANY parcel at all.",
	...
	if MsgContains(message, "undercover") then
		...
		if player:getStorageValue(SPIKE_LOWER_UNDERCOVER_MAIN) == -1 then
			npcHandler:say(
				"Someone is needed to get three reports from our undercover agents posing as monsters in the caves around us. If you are interested, I can give you some more {information} about it. Are you willing to accept this mission?",
				npc,
				creature
			)
	...
	if MsgContains(message, "temperature") then
		...
		if player:getStorageValue(SPIKE_LOWER_LAVA_MAIN) == -1 then
			npcHandler:say(
				"Your task would be to use a gnomish temperature measurement device - short GTMD - to locate the hottest spot at the lava pools in the caves. If you are interested, I can give you some more information about it. Are you willing to accept this mission?",
				npc,
				creature
			)
	...
	if talkState[playerId] == "temperature" then
		if MsgContains(message, "yes") then
			...
			npcHandler:say(
				"Gnometastic! Find the hottest spot of the lava pools in the caves. If you lose the GTMD before you find the hot spot, you'll have to get yourself a new one. Gnomux sells all the equipment that is required for our missions.",
				npc,
				creature
			)
			talkState[playerId] = nil
