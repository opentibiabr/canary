...
local function creatureSayCallback(npc, creature, type, message)
	...
	if talkState[playerId] == "report" then
		...
		elseif MsgContains(message, "fertilisation") then
			if player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Mushroom_Main) == -1 then
				npcHandler:say("You have not started that mission.", npc, creature)
			elseif player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Mushroom_Main) == 4 then
				npcHandler:say("You have done well. Here, take your reward.", npc, creature)
				player:addFamePoint()
				player:addExperience(2000, true)
		...
		elseif MsgContains(message, "killing") then
			if player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Kill_Main) == -1 then
				npcHandler:say("You have not started that mission.", npc, creature)
			elseif player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Kill_Main) == 7 then
				npcHandler:say("You have done well. Here, take your reward.", npc, creature)
				player:addFamePoint()
				player:addExperience(2000, true)
	...
	///////////////////////]]
	if MsgContains(message, "charges") then
		if player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Charge_Daily) >= os.time() then
			return npcHandler:say("Sorry, you have to wait " .. string.diff(player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Charge_Daily) - os.time()) .. " before this task gets available again.", npc, creature)
		end
	...
	if talkState[playerId] == "charges" then
		if MsgContains(message, "yes") then
			...
			player:setStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Charge_Main, 0)
			npcHandler:say(
	...
	if MsgContains(message, "fertilise") then
		if player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Mushroom_Daily) >= os.time() then
			return npcHandler:say("Sorry, you have to wait " .. string.diff(player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Mushroom_Daily) - os.time()) .. " before this task gets available again.", npc, creature)
		end
	...
	if talkState[playerId] == "fertilise" then
		if MsgContains(message, "yes") then
			...
			player:setStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Mushroom_Main, 0)
	...
	if MsgContains(message, "nests") then
		if player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Nest_Daily) >= os.time() then
			return npcHandler:say("Sorry, you have to wait " .. string.diff(player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Nest_Daily) - os.time()) .. " before this task gets available again.", npc, creature)
		end
	...
	if talkState[playerId] == "nests" then
		if MsgContains(message, "yes") then
			player:setStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Nest_Main, 0)
	...
	if MsgContains(message, "kill") then
		if player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Kill_Daily) >= os.time() then
			return npcHandler:say("Sorry, you have to wait " .. string.diff(player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Kill_Daily) - os.time()) .. " before this task gets available again.", npc, creature)
		end
	...
	if talkState[playerId] == "kill" then
		if MsgContains(message, "yes") then
			player:setStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Kill_Main, 0)
