...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "measurements") and npcHandler:getTopic(playerId) == 1 then
		...
		if player:getStorageValue(Storage.DangerousDepths.Gnomes.Measurements) < 1 then
			...
			npcHandler:setTopic(playerId, 2)
		-- Não possuía a missão, agora possui!
		...
		elseif player:getStorageValue(Storage.DangerousDepths.Gnomes.Measurements) == 1 and player:getStorageValue(Storage.DangerousDepths.Gnomes.LocationCount) == 5 then -- Não possuía a missão, agora possui!
			...
			npcHandler:setTopic(playerId, 1)
		end -- Permissão para usar o baú -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1
	elseif npcHandler:getTopic(playerId) == 2 and MsgContains(message, "yes") then
		...
		player:setStorageValue(Storage.DangerousDepths.Gnomes.GnomeChartChest, 1)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.LocationCount, 0)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.LocationA, 0)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.LocationB, 0)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.LocationC, 0)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.LocationD, 0)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.LocationE, 0)
		npcHandler:setTopic(playerId, 1)
	end
	...
	if MsgContains(message, "ordnance") and npcHandler:getTopic(playerId) == 1 then
		...
		elseif player:getStorageValue(Storage.DangerousDepths.Gnomes.Ordnance) == 2 and player:getStorageValue(Storage.DangerousDepths.Gnomes.GnomesCount) >= 5 then -- Não possuía a missão, agora possui!
			if player:getStorageValue(Storage.DangerousDepths.Gnomes.CrawlersCount) >= 3 then
				npcHandler:say(
					{ "AMAZING! Not only did you salve all our friends - you also rescued the animals! Here is your reward and bonus! ...", "The other are already telling stories about you. Please return to me later if you want to help out some more!" },
					npc,
					creature
				)
			...
			npcHandler:setTopic(playerId, 1)
		end -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1
	elseif npcHandler:getTopic(playerId) == 22 and MsgContains(message, "yes") then
		...
		player:setStorageValue(Storage.DangerousDepths.Gnomes.GnomesCount, 0)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.CrawlersCount, 0)
		npcHandler:setTopic(playerId, 1)
	end
	...
	if MsgContains(message, "charting") and npcHandler:getTopic(playerId) == 1 then
		...
		if player:getStorageValue(Storage.DangerousDepths.Gnomes.Charting) < 1 then -- Não possuía a missão, agora possui!
			...
			npcHandler:setTopic(playerId, 33) -- Está na missão porém não terminou a task!
		elseif (player:getStorageValue(Storage.DangerousDepths.Gnomes.Charting) == 1) and (player:getStorageValue(Storage.DangerousDepths.Gnomes.ChartingCount) < 3) then
			npcHandler:say({ "Come back when you have finished your job." }, npc, creature)
			npcHandler:setTopic(playerId, 1)
		end
		if player:getStorageValue(Storage.DangerousDepths.Gnomes.Charting) == 1 and player:getStorageValue(Storage.DangerousDepths.Gnomes.ChartingCount) >= 3 then -- Não possuía a missão, agora possui!
			...
			npcHandler:setTopic(playerId, 1)
		end -- Garantindo que a task não inicie com -1 -- Permissão para usar o papel -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1
	elseif npcHandler:getTopic(playerId) == 33 and MsgContains(message, "yes") then
		...
		player:setStorageValue(Storage.DangerousDepths.Gnomes.ChartingCount, 0)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.GnomeChartPaper, 1)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.OldGate, 0)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.TheGaze, 0)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.LostRuin, 0)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.Outpost, 0)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.Bastion, 0)
		player:setStorageValue(Storage.DangerousDepths.Gnomes.BrokenTower, 0)
		npcHandler:setTopic(playerId, 1)
	end
	local plural = ""
