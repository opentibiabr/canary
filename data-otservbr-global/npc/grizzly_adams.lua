...
npcConfig.shop = { -- HuntsMan rank
-- Sell offers
{
	clientId = 10297,
	sell = 50,
	itemName = "antlers",
	storageKey = POINTSSTORAGE,
	storageValue = 10
}, {
	clientId = 9633,
	sell = 100,
...
local function greetCallback(npc, creature)
	...
	elseif player:getStorageValue(Storage.KillingInTheNameOf.PawAndFurRank) < 0 and player:getStorageValue(POINTSSTORAGE) >= 10 and player:getLevel() >= 6 or player:getStorageValue(Storage.KillingInTheNameOf.PawAndFurRank) == 0 and player:getStorageValue( -- to Huntsman Rank -- to Ranger Rank -- to Big Game Hunter Rank -- to Trophy Hunter Rank
		POINTSSTORAGE
	) >= 20 and player:getLevel() >= 6 or player:getStorageValue(Storage.KillingInTheNameOf.PawAndFurRank) == 2 and player:getStorageValue(POINTSSTORAGE) >= 40 and player:getLevel() >= 50 or player:getStorageValue(
		Storage.KillingInTheNameOf.PawAndFurRank
	) == 4 and player:getStorageValue(POINTSSTORAGE) >= 70 and player:getLevel() >= 80 or player:getStorageValue(Storage.KillingInTheNameOf.PawAndFurRank) == 6 and player:getStorageValue(POINTSSTORAGE) >= 100 and player:getLevel() >= 130 then -- to Elite Hunter Rank
...
local messageBoss =
	{
		...
		},
		{
			...
			"This is a true test of your hunting skills - find it and kill it. Good hunting! ...",
			"You can ask about new {tasks} by the way. Be aware that you can only have one 'Leviathan' task active at the same time!"
		},
		{
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if (MsgContains("join", message) or MsgContains("yes", message)) and npcHandler:getTopic(playerId) == 0 and player:getStorageValue(Storage.KillingInTheNameOf.QuestLogEntry) ~= 0 then
		...
		npcHandler:say("Great! A warm welcome to our newest member: |PLAYERNAME|! Ask me for a {task} if you want to go on a hunt.", npc, creature) -- for death scenario -- for death scenario
	...
	elseif table.contains({ "tasks", "task", "mission" }, message:lower()) then
		...
		if player:getStorageValue(Storage.KillingInTheNameOf.PawAndFurRank) < 0 and player:getStorageValue(POINTSSTORAGE) >= 10 and player:getLevel() >= 6 or player:getStorageValue(Storage.KillingInTheNameOf.PawAndFurRank) == 0 and player:getStorageValue( -- to Huntsman Rank -- to Ranger Rank -- to Big Game Hunter Rank -- to Trophy Hunter Rank
			POINTSSTORAGE
		) >= 20 and player:getLevel() >= 6 or player:getStorageValue(Storage.KillingInTheNameOf.PawAndFurRank) == 2 and player:getStorageValue(POINTSSTORAGE) >= 40 and player:getLevel() >= 50 or player:getStorageValue(
			Storage.KillingInTheNameOf.PawAndFurRank
		) == 4 and player:getStorageValue(POINTSSTORAGE) >= 70 and player:getLevel() >= 80 or player:getStorageValue(Storage.KillingInTheNameOf.PawAndFurRank) == 6 and player:getStorageValue(POINTSSTORAGE) >= 100 and player:getLevel() >= 130 then -- to Elite Hunter Rank
					...
					for j = 1, #tasks.GrizzlyAdams[id].rewards do
						...
						elseif table.contains({ REWARD_POINT, "points", "point" }, reward.type:lower()) and not deny then
							...
							if player:getStorageValue(POINTSSTORAGE) >= 40 and player:getLevel() < 50 or player:getStorageValue(POINTSSTORAGE) >= 70 and player:getLevel() < 80 or player:getStorageValue(POINTSSTORAGE) >= 100 and player:getLevel() < 130 then
								messageAltPoints = true -- fake update -- fake update
							...
							elseif player:getLevel() >= 130 and player:getStorageValue(POINTSSTORAGE) <= 70 then
								...
								messageAltExtraPoints = true
								extraValue = 1
								player:setStorageValue(Storage.KillingInTheNameOf.QuestLogEntry, player:getStorageValue(Storage.KillingInTheNameOf.QuestLogEntry))
							else
		...
		elseif player:getLevel() >= 50 and player:getLevel() < 80 then
			if player:getStorageValue(POINTSSTORAGE) >= 70 then
				...
				npcHandler:say(
					{ "Alright, what would you like to hunt? {Ice golems}, {quara scouts}, {mutated rats}, {ancient scarabs}, {wyverns}, {lancer beetles}, ...", "or {wailing widows}, {killer caimans}, {bonebeasts}, {crystal spiders} or {mutated tigers}." },
					npc,
					creature
				)
			end
		elseif player:getLevel() >= 80 and player:getLevel() < 130 then
			if player:getStorageValue(POINTSSTORAGE) >= 100 then
				...
				npcHandler:say({ "Alright, what would you like to hunt? {Underwater quara}, {giant spiders}, {werewolves}, {nightmares}, {hellspawns}, ...", "as well as {high class lizards}, {stampors}, {brimstone bugs}, {mutated bats}." }, npc, creature)
			end
		elseif player:getLevel() >= 130 and player:getStorageValue(POINTSSTORAGE) < 100 then
			npcHandler:say(
				{ "Alright, what would you like to hunt? You can try {hydras}, {serpent spawns}, {medusae}, {behemoths}, {sea serpents}, ...", "as well as {hellhounds}, {ghastly dragons}, {undead dragons}, {draken} and {destroyers}." },
				npc,
				creature
			)
		else
	...
	elseif message ~= "" and player:canStartTask(message) then
		...
		local messageElse =
			"You already achieved the maximum rank for your level range. If you accept this task, you won't gain points for our society. Hunt " .. tasks.GrizzlyAdams[task].killsRequired .. " " .. tasks.GrizzlyAdams[task].raceName .. " and you'll be rewarded with experience and the possibility to choose a {boss}. Are you in, old chap?"
		...
		elseif table.contains({ "demons", "demon" }, message:lower()) and player:getStorageValue(Storage.KillingInTheNameOf.PawAndFurRank) == 7 then
			npcHandler:say(
				"The spawn of pure evil must be erased from Tibia. You'll find demons lurking in the northern ruins of Edron as well as in some other deeper dungeons of Tibia. Slay 6666 demons for the greater good! Do you think you can handle this task?",
				npc,
				creature
			)
		else
	...
	elseif table.contains({ "promotion", "promotions" }, message:lower()) then
		if player:getStorageValue(Storage.KillingInTheNameOf.PawAndFurRank) < 0 and player:getStorageValue(POINTSSTORAGE) >= 10 and player:getLevel() >= 6 then
			npcHandler:say(
				{ "You gained 10 points! Let me promote you to the first rank: 'Huntsman'. Congratulations! ...", "If you find any trophies - either monster heads or other parts of monsters that you don't need - feel free to ask me for a trade." },
				npc,
				creature
			)
		...
		elseif player:getStorageValue(Storage.KillingInTheNameOf.PawAndFurRank) == 2 and player:getStorageValue(POINTSSTORAGE) >= 40 and player:getLevel() >= 50 then -- to Big Game Hunter Rank
			npcHandler:say(
				{ "Good show! You gained 40 points for the 'Paw and Fur - Hunting Elite'. You have earned the right to join the ranks of those known as 'Big game hunter'. Congratulations! ...", "From now on I'll buy more trophies from you!" },
				npc,
				creature
			)
	...
	elseif table.contains({ "boss", "bosses" }, message:lower()) then
		...
		if player:getStorageValue(Storage.KillingInTheNameOf.BossPoints) > 0 then
			...
			elseif player:getLevel() >= 50 and player:getLevel() < 80 then
				npcHandler:say({ "You can choose between the {Snapper}, {Hide}, {Deathbine}, the {Bloodtusk}, {Shardhead}, {Fleshcrawler}, {Ribstride}, {Bloodweb} and {Esmeralda}" }, npc, creature)
	...
	elseif (getTaskByName(message) and (npcHandler:getTopic(playerId) == 2) and table.contains(getPlayerStartedTasks(creature), getTaskByName(message))) then
		...
		if player:getStorageValue(KillCounter + task) > 0 then
			npcHandler:say(
				"You currently killed " .. player:getStorageValue(
					KillCounter + task
				) .. "/" .. tasks.GrizzlyAdams[task].killsRequired .. " " .. tasks.GrizzlyAdams[task].raceName .. "." .. " " .. "Canceling this task will restart the count." .. " " .. "Are you sure you want to cancel this task?",
				npc,
				creature
			)
		else
	...
	elseif table.contains({ "special", "special task" }, message:lower()) then
		if player:getPawAndFurPoints() >= 70 and player:getLevel() >= 80 then
			...
			elseif player:getStorageValue(Storage.KillingInTheNameOf.MissionDemodras) < 1 then
				npcHandler:say(
					"This task is a very dangerous one. I want you to look for {Demodras'} hideout. It might be somewhere under the {Plains of Havoc}. Good luck, old chap, come back in one piece and ask me about the special task when you're done.",
					npc,
					creature
				)
