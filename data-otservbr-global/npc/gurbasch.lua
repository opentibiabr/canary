...
addTravelKeyword(
	"farmine",
	{ "Do you seek a ride to Farmine for |TRAVELCOST|?", "Hold on!", "You shouldn't miss the experience." },
	110,
	{ "postman", "new frontier" },
	function(player)
		...
		if player:getStorageValue(TheNewFrontier.Mission05[1]) == 2 then --if The New Frontier Quest 'Mission 05: Getting Things Busy' complete then Stage 3
			destination.z = 10 --if The New Frontier Quest 'Mission 03: Strangers in the Night' complete then Stage 2
		elseif player:getStorageValue(TheNewFrontier.Mission03) >= 2 then
			destination.z = 12
		end
		return destination
	end,
...
keywordHandler:addKeyword({ "passage" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "Where do you want me to take you? To {Kazordoon}, {Farmine} or {Gnomprona}?"
})
