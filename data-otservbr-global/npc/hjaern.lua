...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		if player:getStorageValue(Storage.TheIceIslands.Questline) == 3 then
			if player:getStorageValue(Storage.TheIceIslands.Mission02) < 1 then
				...
				player:setStorageValue(Storage.TheIceIslands.Mission02, 1)
		...
		elseif player:getStorageValue(Storage.TheIceIslands.Questline) == 38 then
			...
			player:setStorageValue(Storage.TheIceIslands.Mission12, 1)
			player:addItem(7289, 1)
			npcHandler:setTopic(playerId, 0)
		elseif player:getStorageValue(Storage.TheIceIslands.Questline) == 39 and player:getStorageValue(Storage.TheIceIslands.Obelisk01) == 5 and player:getStorageValue(Storage.TheIceIslands.Obelisk02) == 5 and player:getStorageValue(
			Storage.TheIceIslands.Obelisk03
		) == 5 and player:getStorageValue(Storage.TheIceIslands.Obelisk04) == 5 then
			if player:removeItem(7289, 1) then
				...
				player:setStorageValue(Storage.OutfitQuest.NorsemanAddon, 1)
	...
	-- Questlog The Ice Islands Quest, Nibelor 1: Breaking the Ice
	-- Questlog The Ice Islands Quest, Formorgar Mines 3: The Secret
	-- Questlog The Ice Islands Quest, Formorgar Mines 4: Retaliation
	-- Questlog The Ice Islands Quest, Formorgar Mines 4: Retaliation
	-- Questlog Norseman Outfit Quest
	...
	elseif MsgContains(message, "reward") then
		if player:getStorageValue(Storage.TheIceIslands.Questline) == 41 then
			...
			npcHandler:setTopic(playerId, 3) -- Questlog Norseman Outfit Quest
		elseif player:getStorageValue(Storage.TheIceIslands.Questline) == 43 then
			...
			player:setStorageValue(Storage.OutfitQuest.NorsemanAddon, 3)
			npcHandler:setTopic(playerId, 4)
		end
	elseif MsgContains(message, "tylaf") then
		if player:getStorageValue(Storage.TheIceIslands.Questline) == 36 then
			npcHandler:say(
				{ "You encountered the restless ghost of my apprentice Tylaf in the old mines? We must find out what has happened to him. I enable you to talk to his spirit ...", "Talk to him and then report to me about your mission." },
				npc,
				creature
			)
	...
	elseif MsgContains(message, "cookie") then
		if player:getStorageValue(Storage.WhatAFoolish.Questline) == 31 and player:getStorageValue(Storage.WhatAFoolish.CookieDelivery.Hjaern) ~= 1 then
	...
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			...
			player:setStorageValue(Storage.TheIceIslands.Mission07, 2)
			npcHandler:setTopic(playerId, 0)
