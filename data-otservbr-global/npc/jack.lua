...
local function greetCallback(npc, creature)
	...
	if player:getStorageValue(Storage.TibiaTales.JackFutureQuest.QuestLine) == 7 then
		npcHandler:setMessage(MESSAGE_GREET, "You!! What have you told my family? They are mad at me and I don't even know why! \z
	...
	elseif player:getStorageValue(Storage.TibiaTales.JackFutureQuest.QuestLine) == 10 then
		npcHandler:setMessage(MESSAGE_GREET, "So, you've returned to Spectulus? What did he say, is anything wrong? You have this strange expression \z
...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif MsgContains(message, "furniture") then
		if (player:getStorageValue(Storage.TibiaTales.JackFutureQuest.QuestLine) == 3) then
			if (player:getStorageValue(Storage.TibiaTales.JackFutureQuest.Furniture01) == 1) and (player:getStorageValue(Storage.TibiaTales.JackFutureQuest.Furniture02) == 1) and (player:getStorageValue(
				Storage.TibiaTales.JackFutureQuest.Furniture03
			) == 1) and (player:getStorageValue(Storage.TibiaTales.JackFutureQuest.Furniture04) == 1) and (player:getStorageValue(Storage.TibiaTales.JackFutureQuest.Furniture05) == 1) then
				npcHandler:say(
	...
	elseif MsgContains(message, "yes") then
		...
		if player:getStorageValue(Storage.TibiaTales.JackFutureQuest.QuestLine) == 10 then
			...
			elseif (npcHandler:getTopic(playerId) == 6) then
				npcHandler:say("Yeah, yeah... so what are you still doing here? I guess I... will have to seek out this \z
					Spectulus now, see what he has to say. There is nothing left for me in this place.", npc, creature)
	...
	elseif MsgContains(message, "hobbies") or MsgContains(message, "hobby") then
		...
		elseif (player:getStorageValue(Storage.TibiaTales.JackFutureQuest.QuestLine) == 8) then
			npcHandler:say("I... was... sculpting sulky sculptures. For all my life. Until you came in here and DESTROYED \z
				MY MASTERPIECE. Go away. I don't like you.", npc, creature)
			npcHandler:setTopic(playerId, 0)
		end
	end
	return true
