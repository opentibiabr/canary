...
local function addTravelKeyword(keyword, cost, destination)
	local travelKeyword = keywordHandler:addKeyword({ keyword }, StdModule.say, {
		...
		text = (text or "So you wish for a trip to ") .. keyword:titleCase() .. "? |TRAVELCOST|?",
		cost = cost,
		discount = "postman"
	})
	...
	travelKeyword:addChildKeyword({ "no" }, StdModule.say, {
		npcHandler = npcHandler,
		text = "Well, then you stay here, simple as that.",
		reset = true
	})
end
...
keywordHandler:addKeyword({ "passage" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "Where do you want me to take you? To {Kazordoon} or to {Thais}? Occasionally we can even reach the {mines} if they are cleared."
})
