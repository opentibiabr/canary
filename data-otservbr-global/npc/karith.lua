...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "passage") or MsgContains(message, "sail") then
		...
		elseif player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) >= 5 then
			npcHandler:say({ "For the sake of profit, we established ship routes to {Ab'Dendriel}, {Darashia}, {Venore}, {Ankrahmun}, {Port Hope}, {Thais}, {Liberty Bay} and {Carlin}." }, npc, creature)
			npcHandler:setTopic(playerId, 0)
		else
			return false
		end
	elseif MsgContains(message, "Ab'Dendriel") then
		if player:getStorageValue(Storage.SearoutesAroundYalahar.AbDendriel) ~= 1 and player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) < 5 then
			npcHandler:say({ "I've never been there. I doubt the elves there came up with something noteworthy. Or did you find something interesting there?" }, npc, creature)
	...
	elseif MsgContains(message, "Darashia") then
		if player:getStorageValue(Storage.SearoutesAroundYalahar.Darashia) ~= 1 and player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) < 5 then
			npcHandler:say({ "From all what I have heard, it is an unremarkable pile of huts in the desert. Or did you find something interesting there?" }, npc, creature)
		...
		elseif player:getStorageValue(Storage.SearoutesAroundYalahar.Darashia) == 1 or player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) >= 5 then
			npcHandler:say({ "Of course it is merely superstition that the darashian sand wasp honey brings back youth and vitality, but as long people pay a decent price, I couldn't care less. Do you want a passage to Darashia for 210 gold?" }, npc, creature)
	...
	elseif MsgContains(message, "Venore") then
		if player:getStorageValue(Storage.SearoutesAroundYalahar.Venore) ~= 1 and player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) < 5 then
			npcHandler:say({ "Another port full of smelly humans, fittingly located in a swamp. Or did you find something interesting there?" }, npc, creature)
	...
	elseif MsgContains(message, "Ankrahmun") then
		if player:getStorageValue(Storage.SearoutesAroundYalahar.Ankrahmun) ~= 1 and player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) < 5 then
			npcHandler:say({ "A city full of mad death worshippers, no thanks. Or did you find something interesting there?" }, npc, creature)
		...
		elseif player:getStorageValue(Storage.SearoutesAroundYalahar.Ankrahmun) == 1 or player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) >= 5 then
			npcHandler:say({ "The Yalahari seem to be obsessed with conserving their dead, so I guess the embalming fluid will be a great success in Yalahar. Do you want a passage to Ankrahmun for 230 gold?" }, npc, creature)
	...
	elseif MsgContains(message, "Port Hope") then
		if player:getStorageValue(Storage.SearoutesAroundYalahar.PortHope) ~= 1 and player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) < 5 then
			npcHandler:say({ "Another pointless human settlement. Or did you find something interesting there?" }, npc, creature)
		...
		elseif player:getStorageValue(Storage.SearoutesAroundYalahar.PortHope) == 1 or player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) >= 5 then
			npcHandler:say({ "Ivory is highly prized by the artisans of the Yalahari. Do you want a passage to Port Hope for 260 gold?" }, npc, creature)
	...
	elseif MsgContains(message, "Thais") then
		if player:getStorageValue(Storage.SearoutesAroundYalahar.Thais) ~= 1 and player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) < 5 then
			npcHandler:say({ "Thais must be a hell hole if only half of the stories we hear about it are true. Or did you find something interesting there?" }, npc, creature)
		...
		elseif player:getStorageValue(Storage.SearoutesAroundYalahar.Thais) == 1 or player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) >= 5 then
			npcHandler:say({ "Astonishing enough the royal satin seems to suit the exquisite taste of the Yalahari. Do you want a passage to Thais for 200 gold?" }, npc, creature)
	...
	elseif MsgContains(message, "Liberty Bay") then
		if player:getStorageValue(Storage.SearoutesAroundYalahar.LibertyBay) ~= 1 and player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) < 5 then
			npcHandler:say({ "Which sane captain would sail his ship to a pirate town? Or did you find something interesting there?" }, npc, creature)
	...
	elseif MsgContains(message, "Carlin") then
		if player:getStorageValue(Storage.SearoutesAroundYalahar.Carlin) ~= 1 and player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) < 5 then
			npcHandler:say({ "An unremarkable town compared to the wonders of Yalahar. Or did you find something interesting there?" }, npc, creature)
		...
		elseif player:getStorageValue(Storage.SearoutesAroundYalahar.Carlin) == 1 or player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) >= 5 then
			npcHandler:say({ "The evergreen flower pots are an amusing item that might find some customers here. Do you want a passage to Carlin for 185 gold?" }, npc, creature)
...
keywordHandler:addKeyword({ "kick" }, StdModule.kick, {
	...
	destination = { Position(32811, 31267, 6), Position(32811, 31270, 6), Position(32811, 31273, 6) }
})
...
keywordHandler:addKeyword({ "name" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "I'm Karith. I don't belong to a caste any longer, and only serve the Yalahari."
})
keywordHandler:addKeyword({ "yalahar" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "The city was a marvel to behold. It is certain that it have been the many foreigners that ruined it."
})
...
keywordHandler:addGreetKeyword(
	...
	{
		npcHandler = npcHandler,
		text = "Hello! Tell me what's on your mind. Time is money."
	}
)
--Farewell message
