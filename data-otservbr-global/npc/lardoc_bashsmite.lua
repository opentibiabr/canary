...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "diremaws") and npcHandler:getTopic(playerId) == 1 then
		...
		if player:getStorageValue(Storage.DangerousDepths.Scouts.Diremaw) < 1 then
			npcHandler:say(
				{
					"The gnomes say that these creatures seem to thrive on mushroom sponges. But not only that, apparently they also lay eggs in their own cadavers to breed even faster. ...",
					"Yes, disgusting. I guess it provides everything their offspring needs... well, we probably shouldn't go into that much further. Instead we should focus on preventing that from happening. ... ",
		...
		elseif player:getStorageValue(Storage.DangerousDepths.Scouts.Diremaw) == 1 and player:getStorageValue(Storage.DangerousDepths.Scouts.DiremawsCount) >= 20 then -- Não possuía a missão, agora possui!
			...
			npcHandler:setTopic(playerId, 1)
		end -- Permissão para usar o baú -- Garantindo que a task não inicie com -1
	elseif npcHandler:getTopic(playerId) == 2 and MsgContains(message, "yes") then
		...
		player:setStorageValue(Storage.DangerousDepths.Scouts.GnomishChest, 1)
		player:setStorageValue(Storage.DangerousDepths.Scouts.DiremawsCount, 0)
		npcHandler:setTopic(playerId, 1)
		npcHandler:setTopic(playerId, 1)
	end
	...
	if MsgContains(message, "growth") and npcHandler:getTopic(playerId) == 1 then
		...
		if player:getStorageValue(Storage.DangerousDepths.Scouts.Growth) == 1 and player:getStorageValue(Storage.DangerousDepths.Scouts.BarrelCount) >= 3 then -- Não possuía a missão, agora possui!
			...
			npcHandler:setTopic(playerId, 1)
		end -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1 -- Garantindo que a task não inicie com -1
	elseif npcHandler:getTopic(playerId) == 22 and MsgContains(message, "yes") then
		...
		player:setStorageValue(Storage.DangerousDepths.Scouts.BarrelCount, 0)
		player:setStorageValue(Storage.DangerousDepths.Scouts.FirstBarrel, 0)
		player:setStorageValue(Storage.DangerousDepths.Scouts.SecondBarrel, 0)
		player:setStorageValue(Storage.DangerousDepths.Scouts.ThirdBarrel, 0)
		player:setStorageValue(Storage.DangerousDepths.Scouts.FourthBarrel, 0)
		player:setStorageValue(Storage.DangerousDepths.Scouts.FifthBarrel, 0)
		npcHandler:setTopic(playerId, 1)
		npcHandler:setTopic(playerId, 1)
	end
	local plural = ""
