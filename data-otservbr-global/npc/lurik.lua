...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		if player:getStorageValue(Storage.ExplorerSociety.TheAstralPortals) == 56 and player:getStorageValue(Storage.ExplorerSociety.QuestLine) == 56 then
			...
			npcHandler:setTopic(playerId, 1) -- Questlog The Ice Islands Quest, Formorgar Mines 1: The Mission
		elseif player:getStorageValue(Storage.ExplorerSociety.TheIslandofDragons) == 58 and player:getStorageValue(Storage.ExplorerSociety.QuestLine) == 58 then
			if player:removeItem(7314, 1) then
				npcHandler:say(
					{ "A frozen dragon lord? This is just the information we needed! And you even brought a scale from it! Take these 5000 gold pieces as a reward. ...", "As you did such a great job, I might have another mission for you later." },
					npc,
					creature
				)
		...
		elseif player:getStorageValue(Storage.TheIceIslands.Questline) == 34 and player:getStorageValue(Storage.TheIceIslands.MemoryCrystal) < os.time() then
			...
			player:setStorageValue(Storage.TheIceIslands.Mission09, 1)
		end
		-- ISLAND OF DRAGONS
		-- ISLAND OF DRAGONS -- Questlog The Ice Islands Quest, The Contact -- Questlog The Ice Islands Quest, The Contact
	elseif MsgContains(message, "yes") then
		...
		elseif npcHandler:getTopic(playerId) == 2 then
			...
			player:setStorageValue(Storage.TheIceIslands.Mission08, 2)
			npcHandler:setTopic(playerId, 0)
