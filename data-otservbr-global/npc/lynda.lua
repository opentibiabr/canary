...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif MsgContains(message, "yes") then
		...
		elseif npcHandler:getTopic(playerId) == 3 then
			if player:getItemCount(3065) > 0 and player:getItemCount(3066) > 0 and player:getItemCount(3067) > 0 and player:getItemCount(3069) > 0 and player:getItemCount(3070) > 0 and player:getItemCount(3071) > 0 and player:getItemCount(
				3072
			) > 0 and player:getItemCount(3073) > 0 and player:getItemCount(3074) > 0 and player:getItemCount(3075) > 0 then
...
local function confirmRemoveEngage(npc, creature, message, keywords, parameters, node)
	...
	if playerStatus == PROPOSED_STATUS then
		...
		node:addChildKeyword({ "yes" }, removeEngage, {
			moveup = 3,
			text = "Ok, your marriage proposal to {" .. getPlayerNameById(playerSpouse) .. "} has been removed. Take your wedding ring back."
		})
	else
...
local function confirmDivorce(npc, creature, message, keywords, parameters, node)
	...
	if playerStatus == MARRIED_STATUS then
		...
		node:addChildKeyword({ "no" }, StdModule.say, {
			...
			onlyFocus = true,
			moveup = 3,
			text = "Great! Marriages should be an eternal commitment."
		})
		...
		node:addChildKeyword({ "yes" }, divorce, {
			moveup = 3,
			text = "Ok, you are now divorced of {" .. getPlayerNameById(playerSpouse) .. "}. Think better next time after marrying someone."
		})
	else
...
local node1 = keywordHandler:addKeyword({ "marry" }, StdModule.say, {
	npcHandler = npcHandler,
	onlyFocus = true,
...
local node2 = node1:addChildKeyword({ "yes" }, StdModule.say, {
	npcHandler = npcHandler,
	onlyFocus = true,
...
local node3 = keywordHandler:addKeyword({ "celebration" }, StdModule.say, {
	npcHandler = npcHandler,
	onlyFocus = true,
...
local node4 = node3:addChildKeyword({ "yes" }, StdModule.say, {
	npcHandler = npcHandler,
	onlyFocus = true,
