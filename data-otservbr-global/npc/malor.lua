...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		if player:getStorageValue(Storage.DjinnWar.EfreetFaction.Mission02) == 3 then
			...
			npcHandler:say({ "So you would like to fight for us. Hmm. ...", "You show true courage, human, but I will not accept your offer at this point of time." }, npc, creature)
		end
	...
	elseif MsgContains(message, "task") and player:getStorageValue(Storage.DjinnWar.EfreetFaction.Mission03) == 3 then
		...
		elseif player:getStorageValue(Storage.KillingInTheNameOf.BlueDjinnTask) == 2 then
			npcHandler:say({ "You've met Fahim the Wise? That's good. I'm pretty sure it was easy to give him a nice beating. ...", "If you should feel like killing blue djinns in our service again, just talk to me about that task." }, npc, creature)
...
keywordHandler:addGreetKeyword(
	{ "djanni'hah" },
	{
		...
		text = "Greetings, human |PLAYERNAME|. My patience with your kind is limited, so speak quickly and choose your words well."
	}
)
