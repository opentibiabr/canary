...
npcConfig.outfit = { lookTypeEx = 3114 }
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		...
		elseif player:getStorageValue(Storage.GravediggerOfDrefia.Mission06) == 1 and player:getStorageValue(Storage.GravediggerOfDrefia.Mission07) < 1 then
			npcHandler:say({ "Yes? You have it? You what? Not in the amphoras? You picked it off someone else?!? ...", "Well, the important thing is that you have it. Let's see if it's still in good shape. {Give} it to me." }, npc, creature)
		...
		elseif player:getStorageValue(Storage.GravediggerOfDrefia.Mission07) == 1 and player:getStorageValue(Storage.GravediggerOfDrefia.Mission08) < 1 then
			...
			npcHandler:say(
				{ "I will need brains - don't laugh! Ahem. I will need a stimulated brain, to be precise. ...", "Use two half-eaten brains with the Brain Heater Machine in the Necromancer halls and bring me the fused, stimulated brain. Now go!" },
				npc,
				creature
			)
		...
		elseif player:getStorageValue(Storage.GravediggerOfDrefia.Mission10) == 1 and player:getStorageValue(Storage.GravediggerOfDrefia.Mission11) < 1 then
			...
			npcHandler:say(
				{ "Now that you have shown you've got the brains, I need you to show initiative. ...", "I will need something that can be adequately used as intestines. Something alive. Stuff it into this storage flask and return it to me!" },
				npc,
				creature
			)
		...
		elseif player:getStorageValue(Storage.GravediggerOfDrefia.Mission21) == 1 and player:getStorageValue(Storage.GravediggerOfDrefia.Mission22) < 1 then
			npcHandler:say(
				{ "Yes, I said blood. Important ingredient in necromantic rituals, usually. ...", "You need to spill some vials of blood tincture for this task. Probably means killing blood priests to get those vials. Ready to do this?" },
				npc,
				creature
			)
		...
		elseif player:getStorageValue(Storage.GravediggerOfDrefia.Mission25) == 1 and player:getStorageValue(Storage.GravediggerOfDrefia.Mission26) < 1 then
			...
			npcHandler:say({ "Ah, it is invigorating to fell the dark flows, rushing through Drefia, once again! Capital. This has earned you a reward. ...", "Now, the next mission awaits, {yes}?" }, npc, creature)
		...
		elseif player:getStorageValue(Storage.GravediggerOfDrefia.Mission29) == 1 and player:getStorageValue(Storage.GravediggerOfDrefia.Mission30) < 1 then
			npcHandler:say({ "Ah, finally. Exceptional! Don't you feel the earth awakening to our call? No? Oh. ...", "Anyway, you succeeded in hallowing the Fireglass Altar! Only one altar remains to be hallowed! Shall we {proceed}?" }, npc, creature)
		...
		elseif player:getStorageValue(Storage.GravediggerOfDrefia.Mission33) == 1 and player:getStorageValue(Storage.GravediggerOfDrefia.Mission34) < 1 then
			...
			npcHandler:say({ "Yes! YES! I have felt that! The altars are alive again! Well done! ...", "We are close now. Only one important thing remains: the incantation itself. We need the {scroll} for that." }, npc, creature)
		...
		elseif player:getStorageValue(Storage.GravediggerOfDrefia.Mission73) == 1 then
			npcHandler:say(
				{
	...
	--and player:getStorageValue(Storage.GravediggerOfDrefia.Mission74) < 1 then
	elseif MsgContains(message, "yes") then
		...
		elseif npcHandler:getTopic(playerId) == 6 and player:getStorageValue(Storage.GravediggerOfDrefia.Mission12) == 1 then
			if player:removeItem(19086, 1) then
				...
				npcHandler:say({ "Ah... interesting. A snake? Not bad, not bad at all. ...", "Right. Now, we need to waken the old powers through ritual. Let me know when you are ready for this {mission}." }, npc, creature)
				npcHandler:setTopic(playerId, 0)
			else
		...
		elseif npcHandler:getTopic(playerId) == 13 and player:getStorageValue(Storage.GravediggerOfDrefia.Mission57) == 1 and player:getStorageValue(Storage.GravediggerOfDrefia.Mission58) < 1 then
			if player:removeItem(18933, 1) then
				...
				npcHandler:say({ "Oh, praise the Dark Lord! It is my scroll piece! Give it here! You can have this instead. ...", "Ahh, that feels GOOD. Now, only two pieces left to hunt down! On to the {next} scroll piece, yes?" }, npc, creature)
				npcHandler:setTopic(playerId, 0)
			else
				npcHandler:say("You don't have my scroll.", npc, creature)
				npcHandler:setTopic(playerId, 0)
		...
		elseif npcHandler:getTopic(playerId) == 15 and player:getStorageValue(Storage.GravediggerOfDrefia.Mission68) == 1 and player:getStorageValue(Storage.GravediggerOfDrefia.Mission69) < 1 then
			if player:removeItem(18933, 1) then
				...
				npcHandler:say({ "Ahhh, capital, capital. Good girl for keeping it for me. I'll take back my cape now, thank you. ...", "Now - the final part of the scroll! Ready to go retrieve it?" }, npc, creature)
			else
				npcHandler:say("You don't have my scroll.", npc, creature)
				npcHandler:setTopic(playerId, 0)
			end
