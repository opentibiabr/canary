...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "supplies") then
		if player:getStorageValue(Storage.DjinnWar.EfreetFaction.Mission01) == 1 then
			npcHandler:say({ "What!? I bet, Baa'leal sent you! ...", "I won't tell you anything! Shove off!" }, npc, creature)
	...
	elseif MsgContains(message, "ankrahmun") then
		npcHandler:say({ "Yes, I've lived in Ankrahmun for quite some time. Ahh, good old times! ...", "Unfortunately I had to relocate. <sigh> ...", "Business reasons - you know." }, npc, creature)
	end
	return true
end

keywordHandler:addKeyword({ "prison" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "You mean that's a JAIL? They told me it's the finest hotel in town! THAT explains the lousy roomservice!"
})
keywordHandler:addKeyword({ "jail" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "You mean that's a JAIL? They told me it's the finest hotel in town! THAT explains the lousy roomservice!"
})
