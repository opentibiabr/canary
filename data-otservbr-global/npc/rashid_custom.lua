...
function DeductTournmentCoinsByAccountId(accountId, amount)
	print("rodou")
    local query = string.format("UPDATE `accounts` SET `tournment_coins` = `tournment_coins` - %d WHERE `id` = %d", amount, accountId)
    db.query(query)
end

npcType.onBuyItem = function(npc, player, itemId, subType, amount, ignore, inBackpacks, totalCost)
	print("Vendeu por Tournment!!")
	DeductTournmentCoinsByAccountId(4,5)
end
