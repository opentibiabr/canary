...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif MsgContains(message, "device") then
		if player:getStorageValue(Storage.SeaOfLight.Questline) == 1 then
			npcHandler:say(
				"Persistent little nuisance, aren't we? Well, I like your spirit so I will tell you a secret. I may not look the part but I was once a {scientist}. The academy seemed to not like my... attitude and never actually invited me.",
				npc,
				creature
			)
		end
	...
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			...
			npcHandler:say(
				player:getStorageValue(
					Storage.SeaOfLight.Questline
				) ~= 1 and "Very kind indeed. Maybe you are not such a bad guy after all. Maybe I can even give it back to you one day... you know I was not always like that *mumbles*." or "Thank you very much... plans you say? I don't know what you are talking about. Plans for a magic... device? And the people call ME crazy.",
				npc,
				creature
			)
			npcHandler:setTopic(playerId, 0)
		...
		elseif npcHandler:getTopic(playerId) == 6 then
			npcHandler:say(
				"The device was ultimately destroyed. I barely escaped the chaos with my life. My laboratory was shattered, as were all the components of the cursed device. With nothing left, I started to lead a new, different life. Do you know what that means?",
				npc,
				creature
			)
