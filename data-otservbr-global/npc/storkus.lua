...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		if (player:getStorageValue(Storage.TheInquisition.Questline) == 6) then
			...
			elseif (player:getStorageValue(Storage.TheInquisition.StorkusVampiredust) == 20) then
				...
				player:setStorageValue(Storage.TheInquisition.Mission03, 2)
			end
		elseif (player:getStorageValue(Storage.TheInquisition.Questline) == 7) then
			...
			player:setStorageValue(Storage.TheInquisition.Mission03, 3)
		...
		elseif (player:getStorageValue(Storage.TheInquisition.Questline) == 8 or player:getStorageValue(Storage.TheInquisition.Questline) == 9) then
			if player:removeItem(7924, 1) then
				...
				player:setStorageValue(Storage.TheInquisition.Mission03, 5)
			else
				npcHandler:say("Have ye' killed the vampire lord? Because ye' have no his ring.", npc, creature)
			end
			npcHandler:setTopic(playerId, 0)
		end
	-- The Inquisition Questlog- "Mission 3: Vampire Hunt"
	-- The Inquisition Questlog- "Mission 3: Vampire Hunt"
	-- The Inquisition Questlog- "Mission 3: Vampire Hunt"
	...
	elseif MsgContains(message, "yes") then
		if (npcHandler:getTopic(playerId) == 1) then
			...
			npcHandler:say(
				"Ye've brought me " .. count .. " vampire dusts. " .. (20 - player:getStorageValue(Storage.TheInquisition.StorkusVampiredust)) == 0 and "Ask me for a {mission} to continue your quest." or ("Ye' need to bring " .. (20 - player:getStorageValue(
					Storage.TheInquisition.StorkusVampiredust
				)) .. " more."),
				npc,
				creature
			)
