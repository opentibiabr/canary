...
local function greetCallback(npc, player)
	if player:getStorageValue(Storage.TheIceIslands.HuskyKillStatus) == 1 then
		npcHandler:setMessage(
			MESSAGE_GREET,
			"Iskan told me that you killed huskies here in Svargrond. I will be lenient towards you and won't ban you from Svargrond. But you have to pay me a compensation of 1500 gold for each husky you have killed. Are you willing to pay " .. player:getStorageValue(
				Storage.TheIceIslands.HuskyKill
			) * 1500 .. "?"
		)
	else
...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif MsgContains(message, "mead") then
		if player:getStorageValue(Storage.BarbarianTest.Questline) == 1 then
			...
			npcHandler:setTopic(playerId, 4) -- Questlog Barbarian Test Quest Barbarian Test 2: The Bear Hugging
	...
	elseif MsgContains(message, "no") then
		if player:getStorageValue(Storage.TheIceIslands.HuskyKillStatus) == 1 and npcHandler:getTopic(playerId) == 0 then
			npcHandler:say(
				"I don't know if you realise the consequences. You won't be a member of our community anymore. I ask you for the last time: Are you willing to pay " .. player:getStorageValue(Storage.TheIceIslands.HuskyKill) * 1500 .. " gold as a compensation?",
				npc,
				creature
			)
