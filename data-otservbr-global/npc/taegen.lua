...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		...
		elseif player:getStorageValue(ThreatenedDreams.Mission03[1]) == 2 and player:getItemCount(25782) >= 1 then
			...
			npcHandler:say({ "Thank you! We are so happy. Now Aurita can take a walk on the beach. And I can visit her secret underwater grotto." }, npc, creature)
			npcHandler:setTopic(playerId, 0)
		end
	elseif MsgContains(message, "sun catcher") then
		if player:getStorageValue(ThreatenedDreams.Mission03[1]) == 3 then
			npcHandler:say({ "Have you found some raven herb?" }, npc, creature)
	...
	elseif MsgContains(message, "yes") then
		...
		elseif npcHandler:getTopic(playerId) == 2 then
			if player:getItemCount(5953) > 0 then
				npcHandler:say({ "Thank you, friend! Now I can visit Aurita in her underwater grotto!" }, npc, creature)
				player:removeItem(5953, 1)
				npcHandler:setTopic(playerId, 3)
				player:setStorageValue(ThreatenedDreams.Mission03[1], 4)
			else
				npcHandler:say({ "Please find this plant for me. But know that you'll only find it at night. It resembles a common fern but its leaves are of a lighter green." }, npc, creature)
				npcHandler:setTopic(playerId, 0)
			end
		elseif npcHandler:getTopic(playerId) == 3 then
			if player:getStorageValue(ThreatenedDreams.Mission03.DarkSunCatcher) == 1 then
				npcHandler:say({ "I already crafted one sun catcher for you." }, npc, creature)
			elseif player:getItemCount(675) >= 2 and player:getItemCount(676) >= 2 and player:getItemCount(677) >= 2 and player:getItemCount(678) >= 2 and player:getStorageValue(ThreatenedDreams.Mission03.DarkSunCatcher) < 1 then
				npcHandler:say({ "Alright, I will craft a sun catcher for you." }, npc, creature)
				...
				npcHandler:say({ "I don't have any gems. If you bring me some, I can craft a sun catcher for you. Do you have gems?" }, npc, creature)
				npcHandler:setTopic(playerId, 0)
			end
		end
