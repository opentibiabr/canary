...
local storages = { Storage.FathersBurden.Sinew, Storage.FathersBurden.Wood, Storage.FathersBurden.Cloth, Storage.FathersBurden.Silk, Storage.FathersBurden.Crystal, Storage.FathersBurden.Root, Storage.FathersBurden.Iron, Storage.FathersBurden.Scale }
...
TerebanConfig = {
	...
	["exquisite wood"] = {
		...
		messages = {
			...
			failure = {
				"Sorry, you must be wrong. The wood that I need should be in one of the barbarian camps. ...",
				"It seems logical to search the most northern camp first, it is closest to the place where the tree was cut. Please hurry, there is still so much to do before the birthday."
			},
	...
	["magic crystal"] = {
		...
		messages = {
			...
			failure = {
				"Sorry, you must be wrong. It is probably not easy to find the crystal that we need in such a huge desert which may contain several tombs, but please don't give up. ...",
				"According to my sources the tomb we are looking for is east of Ankrahmun. You probably need a shovel to enter it. We still have some time and I am convinced you will find it."
			},
			no = {
				"It is probably not easy to find the crystal that we need in such a huge desert which may contain several tombs, but please don't give up. ...",
				"According to my sources the tomb we are looking for is east of Ankrahmun. You probably need a shovel to enter it. We still have some time and I am convinced you will find it."
			},
...
function ParseTerebanSay(npc, creature, message, npcHandler)
	...
	if npcHandler:getTopic(playerId) == 0 then
		if MsgContains(message, "cloak") then
			if player:getStorageValue(ThreatenedDreams.Mission01[1]) == 12 then
				...
				player:setStorageValue(ThreatenedDreams.Mission01.FeathersCount, 0)
			else
	...
	-- Start Mission 'Tattered Swan Feathers'
