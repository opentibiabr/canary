...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		...
		end -- Mission 2 end -- Mission 3 start
	...
	elseif MsgContains(message, "something") then
		if npcHandler:getTopic(playerId) == 2 then
			npcHandler:say(
				"Yeah. No one has seen it and lived to tell the tale. People are missing and sometimes there are {traces} of blood or someone heard a scream, but that's all. We have no idea if the killer is a man or a beast, but there is something out there",
				npc,
				creature
			)
	...
	elseif MsgContains(message, "abandoned sewers") then
		if npcHandler:getTopic(playerId) == 4 then
			...
			player:setStorageValue(Storage.DarkTrails.Mission02, 2)
