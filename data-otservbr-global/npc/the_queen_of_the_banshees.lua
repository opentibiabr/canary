...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "seventh seal") then
		npcHandler:say(
			"If you have passed the first six seals and entered the blue fires that lead to \z
	...
	elseif MsgContains(message, "kiss") and npcHandler:getTopic(playerId) == 7 then
		if player:getStorageValue(Storage.Quest.U7_2.TheQueenOfTheBanshees.LastSeal) < 1 then
			npcHandler:say(
				"Are you prepared to receive my kiss, even though this will mean that your \z
					death as well as a part of your soul will forever belong to me, my dear?",
				npc,
				creature
			)
		...
		if player:getStorageValue(Storage.ExplorerSociety.TheSpectralDress) == 48 and player:getStorageValue(Storage.ExplorerSociety.QuestLine) == 48 and player:getStorageValue(Storage.ExplorerSociety.BansheeDoor) < 1 then
			npcHandler:say(
				"Your wish for a spectral dress is silly. \z
					...
					My maidens left one in a box in a room, directly south of here.",
				npc,
				creature
			)
	...
	elseif MsgContains(message, "addon") then
		if player:getStorageValue(Storage.OutfitQuest.WizardAddon) == 5 then
			npcHandler:say(
				"Say... I have been longing for something for an eternity now... \z
					if you help me retrieve it, I will reward you. Do you consent to this arrangement?",
				npc,
				creature
			)
	...
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			if player:getStorageValue(Storage.Quest.U7_2.TheQueenOfTheBanshees.FourthSeal) == 1 then
				npcHandler:say(
					"The Queen of the Banshee: Yessss, I can sense you have passed the seal of sacrifice. \z
						Have you passed any other seal yet?",
					npc,
					creature
				)
		...
		elseif npcHandler:getTopic(playerId) == 2 then
			if player:getStorageValue(Storage.Quest.U7_2.TheQueenOfTheBanshees.FirstSeal) == 1 then
				npcHandler:say(
					"The Queen of the Banshee: I sense you have passed the hidden seal as well. \z
						Have you passed any other seal yet?",
					npc,
					creature
				)
		...
		elseif npcHandler:getTopic(playerId) == 3 then
			if player:getStorageValue(Storage.Quest.U7_2.TheQueenOfTheBanshees.SecondSeal) == 1 then
				npcHandler:say(
					"The Queen of the Banshee: Oh yes, you have braved the plague seal. \z
						Have you passed any other seal yet?",
					npc,
					creature
				)
		...
		elseif npcHandler:getTopic(playerId) == 4 then
			if player:getStorageValue(Storage.Quest.U7_2.TheQueenOfTheBanshees.ThirdSeal) == 1 then
				npcHandler:say(
					"The Queen of the Banshee: Ah, I can sense the power of the seal of \z
						demonrage burning in your heart. Have you passed any other seal yet?",
					npc,
					creature
				)
		...
		elseif npcHandler:getTopic(playerId) == 5 then
			if player:getStorageValue(Storage.Quest.U7_2.TheQueenOfTheBanshees.FifthSeal) == 1 then
				npcHandler:say(
					"The Queen of the Banshee: So, you have managed to pass the seal of the true path. \z
						Have you passed any other seal yet?",
					npc,
					creature
				)
		...
		elseif npcHandler:getTopic(playerId) == 8 then
			if not player:isPzLocked() then
				...
				npcHandler:say(
					"You have spilled too much blood recently and the dead are hungry for your soul. \z
						Perhaps return when you regained you inner balance.",
					npc,
					creature
				)
				npcHandler:setTopic(playerId, 0)
