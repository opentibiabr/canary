...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") and player:getStorageValue(Storage.TibiaTales.ToAppeaseTheMightyQuest) == 2 then
		npcHandler:say({ "You have the smell of the Marid on you. Tell me who sent you?" }, npc, creature)
	...
	if MsgContains(message, "passage") then
		if player:getStorageValue(Storage.DjinnWar.Faction.EfreetDoor) ~= 1 then
			npcHandler:say({ "Only the mighty Efreet, the true djinn of Tibia, may enter Mal'ouquah! ...", "All Marid and little worms like yourself should leave now or something bad may happen. Am I right?" }, npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 1 then
		...
		elseif MsgContains(message, "no") then
			if player:getStorageValue(Storage.DjinnWar.Faction.MaridDoor) == 1 then
				...
				npcHandler:say({ "Of cour... Huh!? No!? I can't believe it! ...", "You... you got some nerves... Hmm. ...", "Maybe we have some use for someone like you. Would you be interested in working for us. Helping to fight the Marid?" }, npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 3 then
		if MsgContains(message, "yes") then
			npcHandler:say({ "Well then - welcome to Mal'ouquah. ...", "Go now to general Baa'leal and don't forget to greet him correctly! ...", "And don't touch anything!" }, npc, creature)
...
keywordHandler:addGreetKeyword(
	{ "djanni'hah" },
	{
		...
		text = "Shove off, little one! Humans are not welcome here, |PLAYERNAME|"
	}
)
