...
local function greetCallback(npc, creature, message)
	...
	if player:getStorageValue(Storage.DjinnWar.Faction.MaridDoor) ~= 1 then
		npcHandler:setMessage(MESSAGE_GREET, { "Whoa? You know the word! Amazing, |PLAYERNAME|! ...", "I should go and tell Fa'hradin. ...", "Well. Why are you here anyway, |PLAYERNAME|?" })
	else
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") and player:getStorageValue(Storage.TibiaTales.ToAppeaseTheMightyQuest) == 1 then
		npcHandler:say({ "I should go and tell Fa'hradin. ...", "I am impressed you know our address of welcome! I honour that. So tell me who sent you on a mission to our fortress?" }, npc, creature)
	...
	if MsgContains(message, "passage") then
		if player:getStorageValue(Storage.DjinnWar.Faction.MaridDoor) ~= 1 then
			npcHandler:say({ "If you want to enter our fortress you have to become one of us and fight the Efreet. ...", "So, are you willing to do so?" }, npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 2 then
		if MsgContains(message, "yes") then
			npcHandler:say({ "Oh. Ok. Welcome then. You may pass. ...", "And don't forget to kill some Efreets, now and then." }, npc, creature)
...
keywordHandler:addGreetKeyword(
	{ "djanni'hah" },
	{
		...
		text = "Whoa! A human! This is no place for you, |PLAYERNAME|. Go and play somewhere else"
	}
)
