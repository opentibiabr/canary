...
local marks = {
	ChildrenOfTheRevolution = { {
		position = Position(33235, 31177, 7),
		type = 4,
		description = "entrance of the camp"
	}, {
		position = Position(33257, 31172, 7),
		type = 3,
		description = "building 1 which you have to spy"
	}, {
		position = Position(33227, 31163, 7),
		type = 3,
		description = "building 2 which you have to spy"
	}, {
		position = Position(33230, 31156, 7),
		type = 3,
		description = "building 3 which you have to spy"
	} },
	WrathOfTheEmperor = { {
		position = Position(33356, 31180, 7),
		description = "tunnel to hideout"
	}, {
		position = Position(33173, 31076, 7),
		description = "the rebel hideout"
	} }
}
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		if player:getStorageValue(Storage.ChildrenoftheRevolution.Questline) < 1 then
			npcHandler:say({ "Zzo you are offering your help to a lizzard? Hmm, unuzzual. I don't know if I can fully truzzt you yet. ...", "You'll have to work to earn my truzzt. Are you zzure you want to offer me your help?" }, npc, creature)
		...
		elseif player:getStorageValue(Storage.ChildrenoftheRevolution.Questline) == 7 then
			if player:getStorageValue(Storage.ChildrenoftheRevolution.SpyBuilding01) == 1 and player:getStorageValue(Storage.ChildrenoftheRevolution.SpyBuilding02) == 1 and player:getStorageValue(Storage.ChildrenoftheRevolution.SpyBuilding03) == 1 then
				npcHandler:say(
					{ "Zzizz izz mozzt unfortunate. Zzo many warriorzz? Large amountzz of food? And how many weaponzz did you zzay? I zzee, hmm. ... ", "Direct confrontation izz futile. We'll have to find ozzer wayzz to approach zze emperor and hizz army. " },
					npc,
					creature
				)
		...
		elseif player:getStorageValue(Storage.ChildrenoftheRevolution.Questline) == 11 then
			npcHandler:say(
				{ "You accomplished an important tazzk in breaking zze defenzze of zze emperor. Not everyzzing might be lozzt yet. ... ", "I've got anozzer mizzion for you which will be even more dangerouzz zzan zze lazzt one. Return to me when you're ready. " },
				npc,
				creature
			)
			npcHandler:setTopic(playerId, 0)
		...
		elseif player:getStorageValue(Storage.WrathoftheEmperor.Questline) == 9 then
			...
			player:setStorageValue(Storage.WrathoftheEmperor.Mission04, 1)
			npcHandler:setTopic(playerId, 0)
		...
		elseif player:getStorageValue(Storage.WrathoftheEmperor.Questline) == 12 then
			...
			player:setStorageValue(Storage.WrathoftheEmperor.Mission05, 1)
			npcHandler:setTopic(playerId, 0)
		end
		-- CHILDREN OF REVOLUTION QUEST
		-- CHILDREN OF REVOLUTION QUEST --Questlog, Children of the Revolution "Prove Your Worzz!" --Questlog, Children of the Revolution "Prove Your Worzz!" --Questlog, Children of the Revolution "Mission 1: Corruption" --Questlog, Children of the Revolution "Mission 2: Imperial Zzecret Weaponzz" --Questlog, Children of the Revolution "Mission 3: Zee Killing Fieldzz" --Questlog, Children of the Revolution "Mission 4: Zze Way of Zztonezz" --Questlog, Children of the Revolution "Mission 4: Zze Way of Zztonezz" --Questlog, Children of the Revolution "Mission 4: Zze Way of Zztonezz" --Questlog, Children of the Revolution "Mission 5: Phantom Army"
		-- CHILDREN OF REVOLUTION QUEST

		-- WRATH OF THE EMPEROR QUEST --Questlog, Wrath of the Emperor "Mission 01: Catering the Lions Den" --Questlog, Wrath of the Emperor "Mission 01: Catering the Lions Den" --Questlog, Wrath of the Emperor "Mission 03: The Keeper" --Questlog, Wrath of the Emperor "Mission 03: The Keeper" --Questlog, Wrath of the Emperor "Mission 04: Sacrament of the Snake"
	...
	--Questlog, Children of the Revolution "Mission 3: Zee Killing Fieldzz"
	--Questlog, Children of the Revolution "Mission 5: Phantom Army"
	-- CHILDREN OF REVOLUTION QUEST

	-- WRATH OF THE EMPEROR QUEST
	--Questlog, Wrath of the Emperor "Mission 04: Sacrament of the Snake"
	--Questlog, Wrath of the Emperor "Mission 05: New in Town"
	-- WRATH OF THE EMPEROR QUEST
	-- WRATH OF THE EMPEROR QUEST
	...
	elseif MsgContains(message, "poison") or MsgContains(message, "poizzon") then
		if player:getStorageValue(Storage.ChildrenoftheRevolution.Questline) == 9 then
			npcHandler:say(
				"Zze emperor of zze dragonzz hazz tranzzformed himzzelf into an undead creature to lazzt for all eternity, to cheat deazz. Hizz corruption flowzz to zzozze he bound, and from zzem to zzozze zzey bound, and from zzem into zze land.",
				npc,
				creature
			)
			npcHandler:setTopic(playerId, 0)
	...
	elseif MsgContains(message, "yes") then
		...
		elseif npcHandler:getTopic(playerId) == 2 then
			...
			player:setStorageValue(Storage.ChildrenoftheRevolution.Mission00, 1)
			npcHandler:setTopic(playerId, 0)
		...
		elseif npcHandler:getTopic(playerId) == 4 then
			if player:removeItem(637, 1) then
				npcHandler:say({ "Aaah, zzezze look zzertainly interezzting. Zzezze manuzzcriptzz show uzz zzeveral locationzz of zze enemy troopzz. ... ", "I'm imprezzed, zzoftzzkin. Maybe you can be of zzome more help. " }, npc, creature)
		...
		elseif npcHandler:getTopic(playerId) == 5 then
			...
			player:setStorageValue(Storage.ChildrenoftheRevolution.Mission01, 1)
			player:addMapMark(Position(33177, 31193, 7), 5, "Temple of Equilibrium")
			npcHandler:setTopic(playerId, 0)
		elseif npcHandler:getTopic(playerId) == 6 then
			npcHandler:say({ "I'll mark zze entranzze to Chaochai on your map. ... ", "Conzzentrate on one location at a time. Zze one who chazzezz after two harezz, won't catch even one. " }, npc, creature)
			player:setStorageValue(Storage.ChildrenoftheRevolution.Questline, 7)
			player:setStorageValue(Storage.ChildrenoftheRevolution.Mission02, 1)
		...
		elseif npcHandler:getTopic(playerId) == 8 then
			npcHandler:say(
				{ "Good. Zze fieldzz should be not far from Xiachai in zze eazzt. Go to zze top terrazze and mix zze poizzon wizz zze water. ... ", "{Poizzon} izz often uzzed by cowardzz, yet it grantzz great power to zze opprezzed. " },
				npc,
				creature
			)
			player:setStorageValue(Storage.ChildrenoftheRevolution.Questline, 9)
			player:setStorageValue(Storage.ChildrenoftheRevolution.Mission03, 1)
			npcHandler:setTopic(playerId, 0)
		elseif npcHandler:getTopic(playerId) == 10 then
			npcHandler:say(
				{ "Perhapzz you can find a way to enter zze norzz of zze valley and find a pazzage to zze great gate itzzelf. Zzearch any templezz or zzettlementzz you come accrozz for hidden pazzagezz. ... ", "I wish for a zzafe return wizz good newzz. " },
				npc,
				creature
			)
			...
			if player:getStorageValue(Storage.ChildrenoftheRevolution.Mission04) <= 1 then
				player:setStorageValue(Storage.ChildrenoftheRevolution.Mission04, 1)
			end
			npcHandler:setTopic(playerId, 0)
		...
		elseif npcHandler:getTopic(playerId) == 13 then
			...
			player:setStorageValue(Storage.ChildrenoftheRevolution.Mission05, 1)
			npcHandler:setTopic(playerId, 0)
		...
		elseif npcHandler:getTopic(playerId) == 15 then
			...
			player:setStorageValue(Storage.WrathoftheEmperor.Mission01, 1)
			npcHandler:setTopic(playerId, 0)
		...
		elseif npcHandler:getTopic(playerId) == 18 then
			if player:getItemCount(953) >= 3 and player:getItemCount(5901) >= 1 then
				...
				player:setStorageValue(Storage.WrathoftheEmperor.Mission01, 2)
		...
		elseif npcHandler:getTopic(playerId) == 19 then
			...
			player:setStorageValue(Storage.WrathoftheEmperor.Mission03, 1)
