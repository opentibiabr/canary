...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		if player:getStorageValue(Storage.WrathoftheEmperor.Questline) == 22 then
			...
			player:setStorageValue(Storage.WrathoftheEmperor.Mission06, 0)
			npcHandler:setTopic(playerId, 0)
		--Questlog, Wrath of the Emperor "Mission 05: New in Town"
		--Questlog, Wrath of the Emperor "Mission 06: The Office Job"
		...
		elseif player:getStorageValue(Storage.WrathoftheEmperor.Questline) == 24 and player:getStorageValue(Storage.WrathoftheEmperor.Mission07) == 6 then
			if npcHandler:getTopic(playerId) ~= 1 then
				...
				npcHandler:setTopic(playerId, 1) --Questlog, Wrath of the Emperor "Mission 08: Uninvited Guests"
			else
				npcHandler:say(
					{
				...
				player:setStorageValue(Storage.WrathoftheEmperor.Mission08, 1)
...
npcHandler:setMessage(MESSAGE_GREET, { "Ah, ze human everyone iz talking about. Your victory over ze champion waz quite imprezzive. ...", "Alzough, for many ziz only provez what a huge zreat you blank-zkinz ztill poze. What do you {want}?" })
