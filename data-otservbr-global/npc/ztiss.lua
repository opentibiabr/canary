...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "guezt") then
		if player:getStorageValue(TheNewFrontier.Questline) == 23 then
			npcHandler:say({ "Ziz iz not for you to azk. I work for zomeone of immenze power. He haz an {offer} for you." }, npc, creature)
	...
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 3 then
			...
			npcHandler:setTopic(playerId, 4) --Questlog, The New Frontier Quest "Mission 08: An Offer You Can't Refuse" --Questlog, The New Frontier Quest "Mission 08: An Offer You Can't Refuse"
		elseif npcHandler:getTopic(playerId) == 4 then
			npcHandler:say(
				{ "Excellent! Now you may leave ziz area zrough ze teleporter to ze norz. It will bring you to a hidden boat. Ziz boat will take you to ze tournament izle. ...", "Zere you'll learn anyzing you need to know about ze great tournament." },
				npc,
				creature
			)
			player:setStorageValue(TheNewFrontier.Questline, 24)
			player:setStorageValue(TheNewFrontier.Mission08, 2)
			player:setStorageValue(TheNewFrontier.Mission09[1], 1)
