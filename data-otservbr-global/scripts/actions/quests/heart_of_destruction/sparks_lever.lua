...
function heartDestructionSparks.onUse(player, item, fromPosition, itemEx, toPosition)
	local config = {
		playerPositions = { Position(32227, 31343, 11), Position(32227, 31344, 11), Position(32227, 31345, 11), Position(32227, 31346, 11), Position(32227, 31347, 11) },
		newPos = {
			x = 32151,
			y = 31301,
			z = 14
		...
		if item.itemid == 8911 then
			if player:getPosition().x == pushPos.x and player:getPosition().y == pushPos.y and player:getPosition().z == pushPos.z then
				...
				if doCheckArea() == false then
					...
					player:say("The room slowly beginns to crackle. An erruption seems imanent!", TALKTYPE_MONSTER_YELL, isInGhostMode, pid, {
						x = 32143,
						y = 31308,
						z = 14
					})
