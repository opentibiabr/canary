...
function theCursedMedusa.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if (item.itemid == 21506) and (item2.itemid == 10420) then
		if (player:getStorageValue(Storage.TibiaTales.TheCursedCrystal.Oneeyedjoe) == 2) then
			if not (((Player(player):getPosition().x - TCC_PILLARPETRIFIED.x) < 5) and ((Player(player):getPosition().x - TCC_PILLARPETRIFIED.x) > -5) and ((Player(player):getPosition().y - TCC_PILLARPETRIFIED.y) < 5) and ((Player(
				player
			):getPosition().y - TCC_PILLARPETRIFIED.y) > -5) and (Player(player):getPosition().z == TCC_PILLARPETRIFIED.z)) then return end
	...
	elseif (item.itemid == 9106) then
		if not Tile(topos) or not Tile(topos):getTopCreature() or not Tile(topos):getTopCreature():isPlayer() then return end
