...
local function checkAndTeleport(areaFromPos, areaToPos, teleportDest, spawnPos)
    ...
    if killCount >= 2 then
        local spectators = Game.getSpectators(areaFromPos, false, true,
            areaToPos.x - areaFromPos.x,
            areaToPos.x - areaFromPos.x,
            areaToPos.y - areaFromPos.y,
            areaToPos.y - areaFromPos.y)
...
function VampireBrideDeath.onDeath(creature, corpse, killer, mostDamage, unjustified, mostDamageKiller)
    ...
    if pos.x >= areaFromPos.x and pos.x <= areaToPos.x and
       pos.y >= areaFromPos.y and pos.y <= areaToPos.y and
       pos.z == areaFromPos.z then
