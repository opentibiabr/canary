...
local function spawnSandMonster(name, _time)
	...
	local randomarea = {
		x = math.random(33092, 33105),
		y = math.random(31853, 31865),
		z = 15
	}
...
function sandkingThink.onThink(creature)
	...
	if str <= 3 then
		if (maxhealth * 0.95) > creature:getHealth() then
			...
			creature:remove()
			local positions = { Position(33095, 31854, 15), Position(33102, 31854, 15), Position(33095, 31864, 15), Position(33102, 31864, 15) }
	...
	elseif str == 4 then
		...
		if ((maxhealth * 0.50) > creature:getHealth()) then
			creature:say("THE SANDKING VANISHES INTO THE SAND AND HIS BROOD EMERGES!", TALKTYPE_MONSTER_SAY)
			creature:remove()
			local ps = { Position(33097, 31857, 15), Position(33099, 31856, 15), Position(33102, 31857, 15) }
			local pass = 0
...
function sandkingDeath.onDeath(creature, attacker, corpse)
	...
	if creature:getName():lower() == "sand brood" then
		addEvent(
			...
			MonsterType(creature:getName()):getCorpseId()
		)
		return true
	end
