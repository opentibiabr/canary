...
function thePrimalMenaceDeath.onDeath(creature, corpse, killer, mostDamage, unjustified, mostDamage_unjustified)
	...
	for key, value in pairs(damageMap) do
		...
		if player and player:getStorageValue(Storage.Quest.U12_90.PrimalOrdeal.ThePrimalMenaceKilled) < 1 then
			player:setStorageValue(Storage.Quest.U12_90.PrimalOrdeal.ThePrimalMenaceKilled, 1)
		end
	end
end
