...
function AccountQuest.hasAccess(player, questId)
    ...
    local query = db.storeQuery(string.format(
        "SELECT 1 FROM `account_quest_access` WHERE `account_id` = %d AND `quest_id` = %s LIMIT 1",
        accountId,
        db.escapeString(normalizedId)
    ))
...
function AccountQuest.unlockAccess(player, questId)
    ...
    return db.query(string.format(
        "INSERT IGNORE INTO `account_quest_access` (`account_id`, `quest_id`, `unlocked_by`) VALUES (%d, %s, %d)",
        accountId,
        db.escapeString(normalizedId),
        player:getGuid()
    ))
...
function AccountQuest.canClaimReward(player, questId)
    ...
    local query = db.storeQuery(string.format(
        "SELECT 1 FROM `account_quest_rewards` WHERE `account_id` = %d AND `quest_id` = %s AND `reward_mode` = %s AND `player_id` = %d LIMIT 1",
        accountId,
        db.escapeString(normalizedId),
        db.escapeString(mode),
        playerId
    ))
...
function AccountQuest.claimReward(player, questId)
    ...
    return db.query(string.format(
        "INSERT IGNORE INTO `account_quest_rewards` (`account_id`, `player_id`, `quest_id`, `reward_mode`, `claimed_by`) VALUES (%d, %d, %s, %s, %d)",
        accountId,
        playerId,
        db.escapeString(normalizedId),
        db.escapeString(mode),
...
function AccountQuest.resetCharacterProgress(player, questId)
    ...
    return true, string.format(
        "Quest '%s' progress was reset for this character. Account access and reward history were preserved.",
        normalizedId
    )
end
