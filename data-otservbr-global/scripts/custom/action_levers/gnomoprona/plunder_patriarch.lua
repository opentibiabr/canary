local config = {
    ...
    bossPosition = Position(33583, 32936, 15), 
    bossArea = {
        fromPos = Position(33572, 32929, 15), 
        toPos = Position(33597, 32970, 15), 
        entrancePos = Position(33586, 32966, 15), 
        exitPosition = Position(33613, 32961, 15) 
    },
    participantsAllowAnyCount = true, 
    participantsPos = {
        Position(33615, 32964, 15),
        Position(33614, 32964, 15),
        Position(33613, 32964, 15), 
        Position(33612, 32964, 15),
        Position(33611, 32964, 15)  
    },
    attempts = {
        level = 100, 
        storage = 710008, 
    ...
    createTeleportPos = Position(33583, 32932, 15), 
    teleportToPosition = Position(33613, 32961, 15), 
...
local plunderPatriarch = Action()

function plunderPatriarch.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    ...
    boss:registerEvent("plunderBossLever")
    ...
    config.kickEventId = addEvent(function ()
        for _, spectator in pairs(getSpectators()) do
            if spectator:isPlayer() then
                ...
                spectator:sendTextMessage(MESSAGE_INFO_DESCR, "It's been a long time and you haven't managed to defeat The Plunder Patriarch.")
            else
                spectator:remove()
            end
        end
...
plunderPatriarch:aid(config.actionId)
plunderPatriarch:register()

local creatureEvent = CreatureEvent("plunderBossLever")
