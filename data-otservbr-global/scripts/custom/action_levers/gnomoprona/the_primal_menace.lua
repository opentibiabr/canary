local config = {
    ...
    bossPosition = Position(33557, 32759, 15), 
    bossArea = {
        fromPos = Position(33551, 32753, 15), 
        toPos = Position(33563, 32765, 15), 
        entrancePos = Position(33549, 32758, 15), 
        exitPosition = Position(33519, 32872, 15) 
    },
    participantsAllowAnyCount = true, 
    participantsPos = {
        Position(33548, 32752, 14),
        Position(33549, 32752, 14),
        Position(33550, 32752, 14), 
        Position(33551, 32752, 14),
        Position(33552, 32752, 14)  
    },
    attempts = {
        level = 100, 
        storage = 710007, 
    ...
    createTeleportPos = Position(33567, 32758, 15), 
    teleportToPosition = Position(33519, 32872, 15), 
...
function brainstealerLever.onUse(player, item, fromPos, target, toPos, isHotkey)
    ...
    boss:registerEvent("primalMenaceBossRoom")
    ...
    config.kickEventId = addEvent(function ()
        for _, spectator in pairs(getSpectators()) do
            if spectator:isPlayer() then
                ...
                spectator:sendTextMessage(MESSAGE_INFO_DESCR, "It's been a long time and you haven't managed to defeat The Primal Menace.")
            else
                spectator:remove()
            end
        end
...
local creatureEvent = CreatureEvent("primalMenaceBossRoom")
