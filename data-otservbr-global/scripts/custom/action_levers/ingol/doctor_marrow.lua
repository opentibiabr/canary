local config = {
    ...
    bossPosition = Position(33837, 32591, 12), 
    bossArea = {
        fromPos = Position(33826, 32583, 12), 
        toPos = Position(33848, 32600, 12), 
        entrancePos = Position(33830, 32591, 12), 
        exitPosition = Position(33809, 32584, 12) 
    },
    participantsAllowAnyCount = true, 
    participantsPos = {
        Position(33812, 32584, 12),
        Position(33811, 32584, 12),
        Position(33810, 32584, 12), 
        Position(33809, 32584, 12),
        Position(33808, 32584, 12)  
    },
    attempts = {
        level = 100, 
        storage = 730507, 
    ...
    createTeleportPos = Position(33829, 32591, 12), 
    teleportToPosition = Position(33809, 32584, 12), 
...
function brainstealerLever.onUse(player, item, fromPos, target, toPos, isHotkey)
    ...
    boss:registerEvent("magmaBubbleBoss")
    ...
    config.kickEventId = addEvent(function ()
        for _, spectator in pairs(getSpectators()) do
            if spectator:isPlayer() then
                ...
                spectator:sendTextMessage(MESSAGE_INFO_DESCR, "It's been a long time and you haven't managed to defeat Doctor Marrow.")
            else
                spectator:remove()
            end
        end
...
local creatureEvent = CreatureEvent("magmaBubbleBoss")
