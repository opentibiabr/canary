local config = {
    actionId = 43014, 
    bossName = "Furious Scoprion",
    bossPosition = Position(32949, 32309, 8), 
    bossArea = {
        fromPos = Position(32942, 32302, 8), 
        toPos = Position(32960, 32316, 8), 
        entrancePos = Position(32959, 32309, 8), 
        exitPosition = Position(32963, 32310, 8) 
    },
    participantsAllowAnyCount = true, 
    participantsPos = {
        Position(32963, 32313, 8),
    },
    attempts = {
        level = 100, 
        storage = 710011, 
    ...
    createTeleportPos = Position(32945, 32313, 8), 
    teleportToPosition = Position(32963, 32310, 8), 
...
function brainstealerLever.onUse(player, item, fromPos, target, toPos, isHotkey)
    ...
    player:setStorageValue(117017, 1)
    ...
    config.kickEventId = addEvent(function ()
        for _, spectator in pairs(getSpectators()) do
            if spectator:isPlayer() then
                ...
                spectator:sendTextMessage(MESSAGE_INFO_DESCR, "It's been a long time and you haven't managed to defeat Furious Scorpion.")
            else
                spectator:remove()
            end
        end
