...
function shopSystemGlobalEvent.onThink(interval)
				...
				if(not item_doesnt_exist) then
					...
					if(full_weight <= free_cap) then
						...
						if(received_item) then
							player:sendTextMessage(messageType, "You received >> ".. add_item_name .." << from Peregrinaje Coins Shop.")
							db.query("DELETE FROM `z_ots_comunication` WHERE `id` = " .. id .. ";")
							db.query("UPDATE `z_shop_history` SET `trans_state`='realized', `trans_real`=" .. os.time() .. " WHERE comunication_id = " .. id .. ";")
							player:save()
							logger.info(">> Shopsystem, added " .. add_item_name .. ".")
							added = true
						else
							player:sendTextMessage(messageType, '>> '.. add_item_name ..' << from Peregrinaje Coins Shop is waiting for you. Please make place for this item in your backpack/hands and wait about '.. interval ..' seconds to get it.')
						end
					else
						player:sendTextMessage(messageType, '>> '.. add_item_name ..' << from Peregrinaje Coins Shop is waiting for you. It weight is '.. (full_weight / 100) ..' oz., you have only '.. (free_cap / 100) ..' oz. free capacity. Put some items in depot and wait about '.. interval ..' seconds to get it.')
					end
				end
	...
	logger.info(">> Shopsystem, added " .. addedItems .. " items. Still waiting with " .. waitingItems .. " items.")
	return true
end

shopSystemGlobalEvent:interval(200000)
shopSystemGlobalEvent:register()

logger.info("Geisor SHOP Revscript")