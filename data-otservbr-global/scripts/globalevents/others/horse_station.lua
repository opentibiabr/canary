	...
	for _, horseInfo in ipairs(horsesToSpawn) do
		for i = 1, horseInfo.amount do
			local spawnPos = Position(
				math.random(spawnAreaPalomino.from.x, spawnAreaPalomino.to.x),
				math.random(spawnAreaPalomino.from.y, spawnAreaPalomino.to.y),
				spawnAreaPalomino.from.z
			)
...
local function spawnAppaloosaHorses()
	...
	for _, horseInfo in ipairs(horsesToSpawn) do
		for i = 1, horseInfo.amount do
			local spawnPos = Position(
				math.random(spawnAreaAppaloosa.from.x, spawnAreaAppaloosa.to.x),
				math.random(spawnAreaAppaloosa.from.y, spawnAreaAppaloosa.to.y),
				spawnAreaAppaloosa.from.z
			)
			Game.createMonster(horseInfo.name, spawnPos, true, true)
		end
	end
