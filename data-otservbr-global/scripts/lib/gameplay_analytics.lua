...
local function sessionInsert(session)
    ...
    return string.format([[INSERT INTO `analytics_sessions`
        ...
        escaped(session.uuid), session.playerId, nameSql, session.vocationId, session.levelStart, session.levelEnd,
        session.startedAt, session.endedAt, math.max(0, session.endedAt - session.startedAt), session.combatSeconds,
        session.experienceRaw, session.experienceFinal, session.damageDealt, session.damageReceived,
        session.healingSelf, session.healingOthers, session.overhealing, session.manaSpent,
        session.monstersKilled, session.deaths, session.lootNpc, session.lootMarket, session.suppliesValue,
        session.partySize, session.sharedExperience and 1 or 0, clampInteger(Analytics.config.detailLevel, 0, 2, 1), Analytics.VERSION)
end
