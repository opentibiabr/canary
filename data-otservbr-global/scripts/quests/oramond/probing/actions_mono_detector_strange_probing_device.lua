...
local function isPositionInArea(pos, area)
    return pos.x >= area.from.x and pos.x <= area.to.x and
           pos.y >= area.from.y and pos.y <= area.to.y and
           pos.z >= area.from.z and pos.z <= area.to.z
end
...
local function getRandomPositionInPrimaryArea()
    ...
    repeat
        randomPos = Position(
            math.random(primary_area.from.x, primary_area.to.x),
            math.random(primary_area.from.y, primary_area.to.y),
            primary_area.from.z
        )
...
local function getRandomPositionInSecondaryArea()
    ...
    repeat
        randomPos = Position(
            math.random(secondary_area.from.x, secondary_area.to.x),
            math.random(secondary_area.from.y, secondary_area.to.y),
            secondary_area.from.z
        )
