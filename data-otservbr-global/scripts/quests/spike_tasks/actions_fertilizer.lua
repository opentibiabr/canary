...
function spikeTasksFertilizer.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if table.contains({ -1, 4 }, player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Mushroom_Main)) then
		return false
	end
	...
	local sum = player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Mushroom_Main) + 1
	player:setStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Mushroom_Main, sum)
