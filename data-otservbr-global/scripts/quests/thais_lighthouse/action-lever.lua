...
function lever.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif item.uid == 30005 then
		local entranceTeleportTile = Tile({
			x = 32232,
			y = 32276,
			z = 9
		})
		local exitTeleportTile = Tile({
			x = 32225,
			y = 32276,
			z = 10
		})
		if item.itemid == 2772 then
			if entranceTeleportTile:getItemById(1949) and exitTeleportTile:getItemById(1949) then
				...
				local teleportEntrance = Game.createItem(1949, 1, {
					x = 32232,
					y = 32276,
					z = 9
				})
				local teleportExit = Game.createItem(1949, 1, {
					x = 32225,
					y = 32276,
					z = 10
				})
				if teleportEntrance and teleportExit then
					teleportEntrance:setDestination({
						x = 32225,
						y = 32274,
						z = 10
					})
					teleportExit:setDestination({
						x = 32230,
						y = 32276,
						z = 9
					})
