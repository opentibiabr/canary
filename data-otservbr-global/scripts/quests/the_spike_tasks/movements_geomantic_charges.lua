...
function geomanticCharges.onStepIn(creature, item, position, fromPosition)
	...
	local tasksLoaded = {}
	if not table.contains({ -1, 8 }, player:getStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Nest_Main)) then
	...
	if tasksLoaded["CHARGE"] then
		...
		player:setStorageValue(Storage.Quest.U10_20.SpikeTaskQuest.Constants.Spike_Middle_Charge_Main, 2)
