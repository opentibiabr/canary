...
-- List generated by Marcosvf132 on 16 June 2022 (12.87)
...
do
	for id, achievTable in ipairs(achievements) do
		if achievTable.name == nil then
			Spdlog.error(string.format("[Achievements registration] - Invalid achievement with no name, id: '%s'",
			id))
			return
		end

		if achievTable.description == nil then
			Spdlog.error(string.format("[Achievements registration] - Invalid achievement with no description, id: '%s'",
			id))
			return
		end

		local secret = false
		if achievTable.secret ~= nil then
			secret = achievTable.secret
		end

		local grade = 0
		if achievTable.grade ~= nil then
			grade = achievTable.grade
		end

		local points = 0
		if achievTable.points ~= nil then
			points = achievTable.points
		end
...
function getAchievementInfoById(id)
	return Game.getAchievementInfoById(id)
end

function getAchievementInfoByName(name)
	return Game.getAchievementInfoByName(name)
end

function getSecretAchievements()
	return Game.getSecretAchievements()
end

function getPublicAchievements()
	return Game.getPublicAchievements()
end
