...
function Hireling:getAvailableOutfits()
	...
	table.insert(outfits, {
		name = HIRELING_OUTFIT_DEFAULT.name,
		lookType = HIRELING_OUTFIT_DEFAULT[sex]
	})
	if flags > 0 then
		local outfit
...
function Hireling:returnToLamp(player_id)
	...
	self.active = 0
	addEvent(
		function(npcId, ownerGuid, hirelingId)
			local npc = Npc(npcId)
			if not npc then
...
function Player:addNewHireling(name, sex)
	...
	if sex == HIRELING_SEX.FEMALE then
		...
		hireling.sex = HIRELING_SEX.FEMALE -- citizen male
	else
	...
	if not saved then
		...
		return false --save hirelingId on item
	else
