...
function HirelingsInit()
	local query = string.format("SELECT `ph`.* FROM `player_hirelings` AS `ph` INNER JOIN `players` as `p` ON `p`.`id` = `ph`.`player_id` WHERE `p`.`worldId` = %d", configManager.getNumber(configKeys.WORLD_ID))
