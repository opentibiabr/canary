...
local function insertPlayerTransactionSummary(player, offer)
	if offer.type == GameStore.OfferTypes.OFFER_TYPE_INSTANT_REWARD_ACCESS then player:addRewardCollectionObtained(math.max(1, offer.count))
	elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_BLESSINGS      then player:addBlessingsObtained(offer.blessid, math.max(1, offer.count))
	elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_HOUSE          then player:addHouseItemsObtained(offer.itemtype, math.max(1, offer.count))
	elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_EXPBOOST       then player:addXpBoostsObtained(1)
	elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_PREYBONUS      then player:addPreyCardsObtained(math.max(1, offer.count))
	elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_HIRELING       then player:addHirelingsObtained(1)
	elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_HIRELING_SKILL       then player:addHirelingJobsObtained(offer.id - HIRELING_STORAGE.SKILL)
	elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_HIRELING_OUTFIT      then player:addHirelingOutfitObtained(offer.id - HIRELING_STORAGE.OUTFIT)
