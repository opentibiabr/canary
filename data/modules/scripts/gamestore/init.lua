...
GameStore.Storages = { expBoostCount = 51052 }
...
GameStore.DefaultValues = { DEFAULT_VALUE_ENTRIES_PER_PAGE = 26 }
...
function parseBuyStoreOffer(playerId, msg)
	...
	if (table.contains(GameStore.OfferTypes, offer.type) == false) or not player or (player:getVocation():getId() == 0) and not GameStore.haveOfferRook( -- we've got an invalid offer type -- player not found -- we don't have such offer -- we could not find the offer -- offer is disabled
		id
	) or not offer or (offer.type == GameStore.OfferTypes.OFFER_TYPE_NONE) or (offer.type ~= GameStore.OfferTypes.OFFER_TYPE_NAMECHANGE and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_EXPBOOST and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_PREYBONUS and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_PREYSLOT and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_TEMPLE and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_SEXCHANGE and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_INSTANT_REWARD_ACCESS and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_POUNCH and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_HIRELING and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_HIRELING_NAMECHANGE and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_HIRELING_SEXCHANGE and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_HIRELING_SKILL and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_HIRELING_OUTFIT and not offer.id) then
...
function Player.canBuyOffer(self, offer)
	...
	if offer.type ~= GameStore.OfferTypes.OFFER_TYPE_NAMECHANGE and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_EXPBOOST and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_PREYSLOT and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_PREYBONUS and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_TEMPLE and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_SEXCHANGE and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_POUNCH and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_HIRELING_SKILL and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_HIRELING_OUTFIT and not offer.id then
		disabled = 1
	end
...
function GameStore.processItemPurchase(player, offerId, offerCount, moveable)
	if player:getFreeCapacity() < ItemType(offerId):getWeight(offerCount) then
		return error({
			code = 0,
			message = "Please make sure you have free capacity to hold this item."
		})
	end
	...
	if inbox and inbox:getEmptySlots() > offerCount then
		...
		return error({
			code = 0,
			message = "Please make sure you have free slots in your store inbox."
		})
	end
end
...
function GameStore.processInstantRewardAccess(player, offerCount)
	if player:getCollectionTokens() + offerCount >= 91 then
		return error({
			code = 1,
			message = "You cannot own more than 90 reward tokens."
		})
	end
...
function GameStore.processOutfitPurchase(player, offerSexIdTable, addon)
	...
	if not looktype then
		return error({
			code = 0,
			message = "This outfit seems not to suit your sex, we are sorry for that!"
		})
	elseif not player:hasOutfit(looktype, 0) and (_addon == 1 or _addon == 2) then
		return error({
			code = 0,
			message = "You must own the outfit before you can buy its addon."
		})
	elseif player:hasOutfit(looktype, _addon) then
		return error({
			code = 0,
			message = "You already own this outfit."
		})
	else
		if not player:addOutfitAddon(looktype, _addon) or not player:hasOutfit(looktype, _addon) then -- TFS call failed -- Additional check; if the looktype doesn't match player sex for example,
			--   then the TFS check will still pass... bug? (TODO)
			error({
				code = 0,
				message = "There has been an issue with your outfit purchase. Your purchase has been cancelled."
			})
		else
...
function GameStore.processMountPurchase(player, offerId)
	if player:hasMount(offerId) then
		return error({
			code = 0,
			message = "You already own this mount."
		})
	end
	...
	if productType == GameStore.ClientOfferTypes.CLIENT_STORE_OFFER_NAMECHANGE then
		...
		if tile then
			if not tile:hasFlag(TILESTATE_PROTECTIONZONE) then
				return error({
					code = 1,
					message = "You can change name only in Protection Zone."
				})
			end
		end
		...
		if resultId ~= false then
			return error({
				code = 1,
				message = "This name is already used, please try again!"
			})
		end
...
function GameStore.processPreyThirdSlot(player)
	if player:preyThirdSlot() then
		return error({
			code = 1,
			message = "You already have unlocked all prey slots."
		})
	end
...
function GameStore.processTaskHuntingThirdSlot(player)
	if player:taskHuntingThirdSlot() then
		return error({
			code = 1,
			message = "You already have unlocked all task hunting slots."
		})
	end
...
function GameStore.processPreyBonusReroll(player, offerCount)
	if player:getPreyCards() + offerCount >= 51 then
		return error({
			code = 1,
			message = "You cannot own more than 50 prey wildcards."
		})
	end
...
function GameStore.processTempleTeleportPurchase(player)
	if player:getCondition(CONDITION_INFIGHT, CONDITIONID_DEFAULT) or player:isPzLocked() then
		return error({
			code = 0,
			message = "You can't use temple teleport in fight!"
		})
	end
...
function GameStore.processHirelingPurchase(player, offer, productType, hirelingName, chosenSex)
	...
	if player:getClient().version < 1200 then
		return error({
			code = 1,
			message = "You cannot buy hirelings on client 10, please relog on client 12 and try again."
		})
	end
	...
	if productType == GameStore.ClientOfferTypes.CLIENT_STORE_OFFER_HIRELING then
		...
		if not hireling then
			return error({
				code = 1,
				message = "Error delivering your hireling lamp, try again later."
			})
		end
		...
		if player:getHirelingsCount() >= 10 then
			return error({
				code = 1,
				message = "You cannot have more than 10 hirelings."
			})
		end
...
function Player:openStore(
serviceName --exporting the method so other scripts can use to open store
)
...
function HandleHirelingNameChange(playerId, offer, newHirelingName)
	...
	player:sendHirelingSelectionModal("Choose a Hireling", "Select a hireling below", cb, {
		offer = offer,
		newHirelingName = newHirelingName
	})
end
