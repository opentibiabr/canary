...
function parseBuyStoreOffer(playerId, msg)
	...
	if (table.contains(GameStore.OfferTypes, offer.type) == false)                      -- we've got an invalid offer type
		...
		or (offer.type ~= GameStore.OfferTypes.OFFER_TYPE_NAMECHANGE and
			...
			offer.type ~= GameStore.OfferTypes.OFFER_TYPE_BUNDLE and
	not offer.id) then
	...
	local pcallOk, pcallError = pcall(function()
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_PREMIUM        then GameStore.processPremiumPurchase(player, offer.validUntil)
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_BUNDLE             then GameStore.processBundlePurchase(player)
...
function sendStoreTransactionHistory(playerId, page, entriesPerPage)
	...
	for k, entry in ipairs(entries) do
		...
		if not oldProtocol then
			msg:addByte(0x1) -- 0 = transferable tibia coin, 1 = normal tibia coin
		end
...
function GameStore.processBundlePurchase(player)
	local inbox = player:getSlotItem(CONST_SLOT_STORE_INBOX)
	if inbox and inbox:getEmptySlots() > 0 then
		...
		if decoKit then
			decoKit:setAttribute(ITEM_ATTRIBUTE_DESCRIPTION, "Você comprou o Bundle Premmium na GameStore para criar um <" .. ItemType(44035):getName() .. ">.")
			decoKit:setCustomAttribute("unWrapId", 44035)
...
function GameStore.processOutfitPurchase(player, offerSexIdTable, addon)
	...
	elseif player:getSex() == PLAYERSEX_FEMALE then
		...
		-- Golden Outfit
		if offerSexIdTable.male == 1210 or offerSexIdTable.female == 1211 then
...
function Player.removeTransferableCoinsBalance(self, coins)
	if self:canRemoveTransferableCoins(coins) then
		return self:setTransferableCoinsBalance(self:getCoinsBalance() - coins)
...
function Player.addTransferableCoinsBalance(self, coins, update)
	self:setTransferableCoinsBalance(self:getCoinsBalance() + coins)
