return function(api)
	...
	function wire.sendBalances(player, state)
		if not api.supportsOfficialTaskboard(player) then
			api.diagnostics.trace("send", "skipped balances for unsupported player='{}'", api.diagnostics.playerName(player))
			return
		end
		local taskPoints = type(player.getTaskHuntingPoints) == "function" and player:getTaskHuntingPoints() or 0
		api.diagnostics.trace("send", "balances player='{}' bountyPoints={} taskPoints={} soulseals={}", api.diagnostics.playerName(player), state.general.bountyPoints, taskPoints, state.general.soulseals)
	...
	function wire.sendBounty(player, state)
		...
		api.diagnostics.trace("send", "window player='{}' opcode=0x{} window={} tasks={} preferences={}", api.diagnostics.playerName(player), api.diagnostics.hexByte(api.packet.serverTaskboard), api.window.bounty, taskCount, preferenceCount)
		msg:sendToPlayer(player)
	...
	function wire.sendWeekly(player, state)
		...
		api.diagnostics.trace("send", "window player='{}' opcode=0x{} window={} kills={} items={} selectionPending={} soulsealsTail={}", api.diagnostics.playerName(player), api.diagnostics.hexByte(api.packet.serverTaskboard), api.window.weekly, killCount, itemCount, state.weekly.selectionPending, soulsealsTail)
		msg:sendToPlayer(player)
		wire.sendBalances(player, state)
	end
	...
	function wire.sendShop(player, state)
		...
		api.diagnostics.trace("send", "window player='{}' opcode=0x{} window={} offers={} wheelMultiplier={} wheelState={}", api.diagnostics.playerName(player), api.diagnostics.hexByte(api.packet.serverTaskboard), api.window.shop, offerCount + 1, wheelMultiplier, wheelState)
		msg:sendToPlayer(player)
		wire.sendBalances(player, state)
	end
