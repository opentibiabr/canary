...
function getBox.onUse(player, box, fromPosition, target, toPosition, isHotkey)
    ...
    elseif item.type == OfferType.ITEM or item.type == OfferType.BACKPACK or item.type == OfferType.HOUSE then
      ...
      if item.type == OfferType.OUTFIT then
        ...
        if player:hasOutfit(female) or player:hasOutfit(male) then
          ...
          createHistory(
            player:getAccountId(),
            boxName .. ' gift, because you already have full NAME outfit',
            item.valueCoins
          )
          table.insert(
            itemsCreated,
            string.format('%d transferable tibia coins, because you already have this outfit.', item.valueCoins)
          )
        else
      ...
      elseif item.type == OfferType.MOUNT then
        if player:hasMount(item.id) then
          ...
          table.insert(
            itemsCreated,
            string.format('%d transferable tibia coins, because you already have this mount.', item.valueCoins)
          )
        else
  ...
  player:sendTextMessage(
    ...
    string.format(
      'Congratulations! \zYou have used %s and won the followed items:\n%s',
      boxName,
      table.concat(itemsCreated, '\n')
    )
  )
