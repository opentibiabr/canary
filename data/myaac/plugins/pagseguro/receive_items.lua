...
function playerLogin.onLogin(player)
  ...
  local resultId = db.storeQuery(
    'SELECT `id`, `item_id`, `item_name`, `item_count` FROM `myaac_send_items` WHERE `account_id` = '
      .. player:getAccountId()
      .. " AND `status` = '1';"
  )
  ...
  if #items > 0 then
    for _, item in ipairs(items) do
      ...
      db.query(
        "UPDATE `myaac_send_items` SET `status` = '2', `updated_at` = "
          .. db.escapeString(updateAt)
          .. ' WHERE `id` = '
          .. item.transaction_id
          .. ';'
      )
    ...
    player:sendTextMessage(
      ...
      string.format('Congratulations! \zYou have received the followed items:\n%s', table.concat(itemsCreated, '\n'))
    )
