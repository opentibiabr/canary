	...
	function QTip(target, options, id) {
		...
		$.extend(self, self.options.api, {
			...
			updatePosition: function (event, animate) {
				...
				if(event && (/(move|in|out|enter|leave)$/).test(event.type) && options.position.target === 'mouse') {
				...
				if(posOptions.adjust.screen) {
					(function() {
						...
						if(self.elements.tip && tip) {
							...
							if(tip.x !== 'center' && adapted.x > -1) {
								tip.x = adjusted.x = tip.x === 'left' ? 'right' : 'left';
							}
							if(tip.y !== 'center' && adapted.y > -1) {
								tip.y = adjusted.y = tip.y === 'top' ? 'bottom' : 'top';
							}
				...
				if(animate === true) {
					...
					self.elements.tooltip.animate(position, 200, 'swing', function () {
