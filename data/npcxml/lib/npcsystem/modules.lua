...
if Modules == nil then
		...
		for item in string.gmatch(data, "[^;]+") do
			...
			for temp in string.gmatch(item, "[^,]+") do
				...
				elseif i == 5 then
					...
					Spdlog.warn("[ShopModule:parseBuyable] - " .. NpcOld():getName() .. "] NpcSystem: Unknown parameter found in buyable items parameter.", temp, item)
				end
				i = i + 1
			end
			...
			if SHOPMODULE_MODE == SHOPMODULE_MODE_TRADE then
				if itemid and cost then
					if subType == nil and it:isFluidContainer() then
						Spdlog.warn("[ShopModule:parseBuyable] - " .. NpcOld():getName() .. "] NpcSystem: SubType missing for parameter item:", item)
					else
						self:addBuyableItem(nil, itemid, cost, subType, realName)
					end
				else
					Spdlog.warn("[ShopModule:parseBuyable] - " .. NpcOld():getName() .. "] NpcSystem: Parameter(s) missing for item:", itemid, cost)
				end
			else
				if name and itemid and cost then
					...
					Spdlog.warn("[ShopModule:parseBuyable] - " .. NpcOld():getName() .. "] NpcSystem: Parameter(s) missing for item:", name, itemid, cost)
				end
			end
		end
	end
		...
		for item in string.gmatch(data, "[^;]+") do
			...
			for temp in string.gmatch(item, "[^,]+") do
				...
				elseif i == 5 then
					...
					Spdlog.warn("[ShopModule:parseSellable] - " .. NpcOld():getName() .. "] NpcSystem: Unknown parameter found in sellable items parameter.", temp, item)
				end
				i = i + 1
			end
			...
			if SHOPMODULE_MODE == SHOPMODULE_MODE_TRADE then
				if itemid and cost then
					...
					Spdlog.warn("[ShopModule:parseSellable] - " .. NpcOld():getName() .. "] NpcSystem: Parameter(s) missing for item:", itemid, cost)
				end
			else
				if name and itemid and cost then
					local names = {}
					...
					Spdlog.warn("[ShopModule:parseSellable] - " .. NpcOld():getName() .. "] NpcSystem: Parameter(s) missing for item:", name, itemid, cost)
				end
			end
		end
	end
		...
		for item in string.gmatch(data, "[^;]+") do
			...
			for temp in string.gmatch(item, "[^,]+") do
				...
				elseif i == 6 then
					...
					Spdlog.warn("[ShopModule:parseBuyableContainers] - " .. NpcOld():getName() .. "] NpcSystem: Unknown parameter found in buyable items parameter.", temp, item)
				end
				i = i + 1
			end
			...
			if name and container and itemid and cost then
				if subType == nil and ItemType(itemid):isFluidContainer() then
					Spdlog.warn("[ShopModule:parseBuyableContainers] - " .. NpcOld():getName() .. "] NpcSystem: SubType missing for parameter item:", item)
				else
					local names = {}
					names[#names + 1] = name
				...
				Spdlog.warn("[ShopModule:parseBuyableContainers] - " .. NpcOld():getName() .. "] NpcSystem: Parameter(s) missing for item:", name, container, itemid, cost)
			end
		end
	end
	...
	function ShopModule:callbackOnBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
		...
		if a < amount then
			...
			if a > 0 then
				if not player:removeMoneyBank((a * shopItem.buy) + (b * 20)) then
					return false
				end
				return true
			end
	...
	function ShopModule.onConfirm(cid, message, keywords, parameters, node)
		...
		elseif shop_eventtype[cid] == SHOPMODULE_BUY_ITEM then
			...
			if a < shop_amount[cid] then
				...
				module.npcHandler:say(msg, cid)
				if a > 0 then
					if not player:removeMoneyBank(a * shop_cost[cid]) then
						return false
					end
