...
local usablesingeingsteedItems = Action()

function usablesingeingsteedItems.onUse(player, item, fromPosition, target, toPosition, isHotkey)
...
for itemId, _ in pairs(config.requiredItems) do
	usablesingeingsteedItems:id(itemId)
end

usablesingeingsteedItems:register()
