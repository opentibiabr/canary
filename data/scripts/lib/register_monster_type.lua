...
registerMonsterType.bosstiary = function(mtype, mask)
	...
	if mask.bosstiary then
		...
		if bossClass == nil then
			logger.error(string.format("Attempting to register a bosstiary boss without a race. Boss name: %s", mtype:name()))
			return
		end
		if mask.bosstiary.bossRaceId then
			mtype:bossRaceId(mask.bosstiary.bossRaceId)
		else
			logger.error(string.format("Attempting to register a bosstiary boss without a raceId. Boss name: %s", mtype:name()))
		end
