local httpclient = TalkAction("/httpclient")
...
local function parseJson(bodyData)
	...
	local status, data = pcall( function() return json.decode(bodyData) end )
...
function httpclient.onSay(player, words, param)
...
httpclient:separator(" ")
httpclient:groupType("god")
httpclient:register()