...
function info.onSay(player, words, param)
    ...
    local targetIpString = target:getIpString()
    ...
    text = text .. "Position: " .. string.format(
        "(%0.5d / %0.5d / %0.3d)",
        target:getPosition().x,
        target:getPosition().y,
        target:getPosition().z
    ) .. "\n"
    text = text .. "IP: " .. Game.convertIpToString(targetIp, targetIpString) .. "\n\n"
    ...
    for _, targetPlayer in ipairs(Game.getPlayers()) do
        if targetPlayer ~= target then
            local playerIpString = targetPlayer:getIpString()
            if playerIpString ~= "" and playerIpString == targetIpString then
