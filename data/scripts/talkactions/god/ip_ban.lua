...
function ipBan.onSay(player, words, param)
	...
	db.query(string.format("INSERT INTO `ip_bans` (`ip`, `reason`, `banned_at`, `expires_at`, `banned_by`) " .. "VALUES (%s, '', %d, %d, %d)", db.escapeString(targetIp), timeNow, timeNow + (ipBanDays * 86400), player:getGuid()))
