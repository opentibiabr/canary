	...
	CROW_ROUTE(app, "/api/v1/players/<string>")
		.methods("GET"_method)([](const crow::request& req, const std::string& playerName) {
			try {
				...
				// Busca o jogador
				auto response = PlayerEndpoints::getPlayerInfo(playerName);
				if (response.code == 404) {
					// Converte 404 para uma resposta mais amigável
					return APIResponse::notFound("Jogador não encontrado: " + Validators::urlDecode(playerName));
				}
				return response;
			} catch (const std::exception& e) {
...
bool APIServer::isNewerVersion(const std::string& current, const std::string& new_version) {
	...
	for (size_t i = 0; i < 3; i++) {
		if (new_parts[i] > curr_parts[i]) return true;
		if (new_parts[i] < curr_parts[i]) return false;
	}
...
std::map<std::string, std::vector<VersionInfo>> APIServer::availableVersions = {
	{"android", {
	        {
	        	"1.1.0",
				"https://github.com/beats-dh/canary/releases/download/v1.4.2/app-release.apk",
				"- Melhorias na interface\n- Correção de bugs de conexão\n- Novo sistema de atualização\n- Fixs em varias extruturas",
				false
			}
	}}
};
