...
std::shared_ptr<Aura> Attachedeffects::getAuraByID(uint8_t id) {
	auto it = std::find_if(auras.begin(), auras.end(), [id](const std::shared_ptr<Aura> &aura) {
		return aura->id == id;
	});
...
std::shared_ptr<Effect> Attachedeffects::getEffectByID(uint8_t id) {
	auto it = std::find_if(effects.begin(), effects.end(), [id](const std::shared_ptr<Effect> &effect) {
		return effect->id == id;
	});
...
std::shared_ptr<Wing> Attachedeffects::getWingByID(uint8_t id) {
	auto it = std::find_if(wings.begin(), wings.end(), [id](const std::shared_ptr<Wing> &wing) {
		return wing->id == id;
	});
...
std::shared_ptr<Shader> Attachedeffects::getShaderByID(uint8_t id) {
	auto it = std::find_if(shaders.begin(), shaders.end(), [id](const std::shared_ptr<Shader> &shader) {
		return shader->id == id;
	});
...
std::shared_ptr<Aura> Attachedeffects::getAuraByName(const std::string &name) {
	...
	auto it = std::find_if(auras.begin(), auras.end(), [auraName](const std::shared_ptr<Aura> &aura) {
...
std::shared_ptr<Shader> Attachedeffects::getShaderByName(const std::string &name) {
	...
	auto it = std::find_if(shaders.begin(), shaders.end(), [shaderName](const std::shared_ptr<Shader> &shader) {
...
std::shared_ptr<Effect> Attachedeffects::getEffectByName(const std::string &name) {
	...
	auto it = std::find_if(effects.begin(), effects.end(), [effectName](const std::shared_ptr<Effect> &effect) {
...
std::shared_ptr<Wing> Attachedeffects::getWingByName(const std::string &name) {
	...
	auto it = std::find_if(wings.begin(), wings.end(), [wingName](const std::shared_ptr<Wing> &wing) {
