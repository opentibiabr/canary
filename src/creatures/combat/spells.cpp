...
bool Spell::playerSpellCheck(std::shared_ptr<Player> player) const {
	...
	if (aggressive && (range < 1 || (range > 0 && !player->getAttackedCreature())) && player->getSkull() == Skull_t::Black) {
		player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
		return false;
	}

	if (aggressive && player->hasCondition(ConditionType::Pacified)) {
		player->sendCancelMessage(RETURNVALUE_YOUAREEXHAUSTED);
		g_game().addMagicEffect(player->getPosition(), CONST_ME_POFF);
		return false;
	}

	if (aggressive && !player->hasFlag(PlayerFlags_t::IgnoreProtectionZone) && player->getZoneType() == ZoneType::Protection) {
		player->sendCancelMessage(RETURNVALUE_ACTIONNOTPERMITTEDINPROTECTIONZONE);
		return false;
	}

	if (player->hasCondition(ConditionType::SpellGroupCooldown, group) || player->hasCondition(ConditionType::SpellCooldown, m_spellId) || (secondaryGroup != SPELLGROUP_NONE && player->hasCondition(ConditionType::SpellGroupCooldown, secondaryGroup))) {
		player->sendCancelMessage(RETURNVALUE_YOUAREEXHAUSTED);
...
bool Spell::playerRuneSpellCheck(std::shared_ptr<Player> player, const Position &toPos) {
	...
	if (aggressive && needTarget && topVisibleCreature && player->hasSecureMode()) {
		...
		if (targetPlayer && targetPlayer != player && player->getSkullClient(targetPlayer) == Skull_t::None && !Combat::isInPvpZone(player, targetPlayer)) {
...
void Spell::applyCooldownConditions(std::shared_ptr<Player> player) const {
	...
	if (cooldown > 0) {
		...
		if (spellCooldown > 0) {
			std::shared_ptr<Condition> condition = Condition::createCondition(ConditionId_t::Default, ConditionType::SpellCooldown, spellCooldown / rateCooldown, 0, false, m_spellId);
	...
	if (groupCooldown > 0) {
		...
		if (spellGroupCooldown > 0) {
			std::shared_ptr<Condition> condition = Condition::createCondition(ConditionId_t::Default, ConditionType::SpellGroupCooldown, spellGroupCooldown / rateCooldown, 0, false, group);
			player->addCondition(condition);
		}
	}
	...
	if (secondaryGroupCooldown > 0) {
		...
		if (spellSecondaryGroupCooldown > 0) {
			std::shared_ptr<Condition> condition = Condition::createCondition(ConditionId_t::Default, ConditionType::SpellGroupCooldown, spellSecondaryGroupCooldown / rateCooldown, 0, false, secondaryGroup);
			player->addCondition(condition);
		}
	}
}
