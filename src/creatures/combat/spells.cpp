...
{
	if (cooldown > 0) {
		Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_SPELLCOOLDOWN, cooldown / static_cast<int32_t>(g_config.getFloat(RATE_SPELL_COOLDOWN)), 0, false, spellId);
		player->addCondition(condition);
	}

	if (groupCooldown > 0) {
		Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_SPELLGROUPCOOLDOWN, groupCooldown / static_cast<int32_t>(g_config.getFloat(RATE_SPELL_COOLDOWN)), 0, false, group);
		player->addCondition(condition);
	}

	if (secondaryGroupCooldown > 0) {
		Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_SPELLGROUPCOOLDOWN, secondaryGroupCooldown / static_cast<int32_t>(g_config.getFloat(RATE_SPELL_COOLDOWN)), 0, false, secondaryGroup);
		player->addCondition(condition);
	}
}
