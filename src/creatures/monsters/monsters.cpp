...
bool Monsters::loadFromXml(bool reloading /*= false*/)
{
	...
	for (auto monsterNode : doc.child("monsters").children()) {
		...
		std::string file = g_configManager().getString(DATAPACK_FOLDER_NAME) + "/monster/" + std::string(monsterNode.attribute("file").as_string());
...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb, const std::string& description)
{
	...
	if (isScripted) {
		...
		if (!combatSpellPtr->loadScript(g_configManager().getString(DATAPACK_FOLDER_NAME) + "/" + g_spells().getScriptBaseName() + "/scripts/" + scriptName)) {
			return false;
		}
...
bool Monsters::deserializeSpell(MonsterSpell* spell, spellBlock_t& sb, const std::string& description)
{
	...
	if (spell->isScripted) {
		...
		if (!combatSpellPtr->loadScript(g_configManager().getString(DATAPACK_FOLDER_NAME) + "/" + g_spells().getScriptBaseName() + "/scripts/" + spell->scriptName)) {
...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading /*= false*/)
{
	...
	if ((attr = monsterNode.attribute("script"))) {
		...
		if (scriptInterface->loadFile(g_configManager().getString(DATAPACK_FOLDER_NAME) + "/monster/scripts/" + script) == 0) {
