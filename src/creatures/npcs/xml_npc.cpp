...
uint32_t NpcOld::npcOldAutoID = 0x90000000;
NpcScriptInterface* NpcOld::scriptInterface = nullptr;

void NpcsOld::reload()
{
	const std::map<uint32_t, NpcOld*>& npcsOld = g_game.getNpcsOld();
	for (const auto& it : npcsOld) {
		it.second->closeAllShopWindows();
	}

	delete NpcOld::scriptInterface;
	NpcOld::scriptInterface = nullptr;
...
NpcOld* NpcOld::createNpc(const std::string& name)
{
	std::unique_ptr<NpcOld> npcOld(new NpcOld(name));
	if (!npcOld->load()) {
		return nullptr;
	}
	return npcOld.release();
}

NpcOld::NpcOld(const std::string& initName) :
	Creature(),
	filename("data/npcxml/" + initName + ".xml"),
...
void NpcOld::addList()
{
	g_game.addNpcOld(this);
}

void NpcOld::removeList()
{
	g_game.removeNpcOld(this);
}
...
bool NpcOld::load()
{
	...
	if (!scriptInterface) {
		...
		scriptInterface->loadNpcLib("data/npcxml/lib/npc.lua");
	}

	return setLoaded(loadFromXml());
}
...
bool NpcOld::loadFromXml()
{
	pugi::xml_document doc;
	pugi::xml_parse_result result = doc.load_file(filename.c_str());
	if (!result) {
		printXMLError("[NpcOld::loadFromXml] - {} {}", filename, result);
		return false;
	}

	pugi::xml_node npcNode = doc.child("npc");
	if (!npcNode) {
		SPDLOG_ERROR("[NpcOld::loadFromXml] - Missing npc tag in {}", filename);
		return false;
	}
...
bool NpcOld::canSee(const Position& pos) const
{
...
std::string NpcOld::getDescription(int32_t) const
{
	std::string descr;
...
void NpcOld::onCreatureAppear(Creature* creature, bool isLogin)
{
...
void NpcOld::onRemoveCreature(Creature* creature, bool isLogout)
{
...
void NpcOld::onCreatureMove(Creature* creature, const Tile* newTile, const Position& newPos,
...
void NpcOld::onCreatureSay(Creature* creature, SpeakClasses type, const std::string& text)
{
...
void NpcOld::onPlayerCloseChannel(Player* player)
{
	if (npcEventHandler) {
...
void NpcOld::onThink(uint32_t interval)
{
...
void NpcOld::doSay(const std::string& text)
{
	g_game.internalCreatureSay(this, TALKTYPE_SAY, text, false);
}

void NpcOld::doSayToPlayer(Player* player, const std::string& text)
{
	if (player) {
...
void NpcOld::onPlayerTrade(Player* player, int32_t callback, uint16_t itemId, uint8_t count,
...
void NpcOld::onPlayerEndTrade(Player* player, int32_t buyCallback, int32_t sellCallback)
{
...
bool NpcOld::getNextStep(Direction& dir, uint32_t& flags)
{
...
bool NpcOld::canWalkTo(const Position& fromPos, Direction dir) const
{
	if (masterRadius == 0) {
		return false;
	}
	...
	if (!SpawnsNpcOld::isInZone(masterPos, masterRadius, toPos)) {
		return false;
	}
...
bool NpcOld::getRandomStep(Direction& dir) const
{
...
void NpcOld::doMoveTo(const Position& target)
{
...
void NpcOld::turnToCreature(Creature* creature)
{
...
void NpcOld::setCreatureFocus(Creature* creature)
{
	if (creature) {
...
void NpcOld::addShopPlayer(Player* player)
{
	oldShopPlayerSet.insert(player);
}

void NpcOld::removeShopPlayer(Player* player)
{
	oldShopPlayerSet.erase(player);
}

void NpcOld::closeAllShopWindows()
{
	while (!oldShopPlayerSet.empty()) {
		Player* player = *oldShopPlayerSet.begin();
		if (!player->closeOldShopWindow()) {
			removeShopPlayer(player);
		}
	}
}

NpcScriptInterface* NpcOld::getScriptInterface()
{
	return scriptInterface;
}

NpcScriptInterface::NpcScriptInterface() :
	LuaScriptInterface("NpcOld interface")
{
	libLoaded = false;
	initState();
}
...
int NpcScriptInterface::luaActionSay(lua_State* L)
{
	...
	if (lua_gettop(L) >= 2) {
		...
		if (target) {
			npcOld->doSayToPlayer(target, text);
			return 0;
		}
	}
...
int NpcScriptInterface::luaActionMove(lua_State* L)
{
	...
	NpcOld* npcOld = getScriptEnv()->getNpcOld();
	if (npcOld) {
		g_game.internalMoveCreature(npcOld, getNumber<Direction>(L, 1));
	}
	return 0;
}
...
int NpcScriptInterface::luaActionTurn(lua_State* L)
{
	...
	NpcOld* npcOld = getScriptEnv()->getNpcOld();
	if (npcOld) {
		g_game.internalCreatureTurn(npcOld, getNumber<Direction>(L, 1));
	}
	return 0;
}
...
int NpcScriptInterface::luaActionFollow(lua_State* L)
{
	...
	pushBoolean(L, npcOld->setFollowCreature(getPlayer(L, 1)));
	return 1;
}
...
int NpcScriptInterface::luagetDistanceTo(lua_State* L)
{
	...
	const Position& npcPos = npcOld->getPosition();
...
int NpcScriptInterface::luaSetNpcFocus(lua_State* L)
{
	...
	NpcOld* npcOld = getScriptEnv()->getNpcOld();
	if (npcOld) {
		npcOld->setCreatureFocus(getCreature(L, -1));
	}
	return 0;
}
...
int NpcScriptInterface::luaGetNpcParameter(lua_State* L)
{
	...
	auto it = npcOld->parameters.find(paramKey);
...
{
	//openOldShopWindow(cid, items, onBuy callback, onSell callback)
...
NpcEventsHandler::NpcEventsHandler(const std::string& file, NpcOld* npcEvent) :
	npcOld(npcEvent), scriptInterface(npcEvent->getScriptInterface())
{
	loaded = scriptInterface->loadFile("data/npcxml/scripts/" + file, npcOld) == 0;
	loaded = scriptInterface->loadFile("data/npcxml/scripts/" + file, npcOld) == 0;
	if (!loaded) {
...
void NpcEventsHandler::onCreatureAppear(Creature* creature)
{
	...
	if (!scriptInterface->reserveScriptEnv()) {
		SPDLOG_ERROR("[NpcScript::onCreatureAppear - NPC {} creature {}] "
                     ...
                     npcOld->getName(), creature->getName());
		return;
	}
