...
struct TitleStorage {
	...
	TitleStorage(uint16_t key, std::string kvKey, uint32_t value) :
		key(key), kvKey(std::move(kvKey)), value(value) { }
};
...
struct Title {
	...
	Title(uint8_t id, CyclopediaTitle_t type, std::string maleName, std::string description, TitleStorage titleStorage, bool permanent) :
		m_id(id), m_type(type), m_maleName(std::move(maleName)), m_description(std::move(description)), m_storage(std::move(titleStorage)), m_permanent(permanent) { }
