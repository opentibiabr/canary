...
Party::Party(const std::shared_ptr<Player> &initLeader) :
	m_leader(initLeader) {
	initLeader->setParty(this);
...
bool Party::passPartyLeadership(const std::shared_ptr<Player> &player) {
	auto leader = getLeader();
	if (!leader || !player || leader == player || player->getParty() != this) {
		return false;
	}
...
void Party::updatePlayerHealth(const std::shared_ptr<Player> &player, std::shared_ptr<Creature> target, uint8_t healthPercent) {
	auto leader = getLeader();
	if (!leader) {
		return;
	}
...
void Party::addPlayerLoot(const std::shared_ptr<Player> &player, std::shared_ptr<Item> item) {
	auto leader = getLeader();
	if (!leader) {
		return;
	}
...
void Party::addPlayerSupply(const std::shared_ptr<Player> &player, std::shared_ptr<Item> item) {
	auto leader = getLeader();
	if (!leader) {
		return;
	}
