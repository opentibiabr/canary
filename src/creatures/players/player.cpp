...
bool Player::canWalkthrough(std::shared_ptr<Creature> creature) {
	...
	if (player) {
		...
		if (!playerTile || (!playerTile->hasFlag(TILESTATE_NOPVPZONE) && !playerTile->hasFlag(TILESTATE_PROTECTIONZONE) && player->getLevel() > static_cast<uint32_t>(g_configManager().getNumber(PROTECTION_LEVEL, __FUNCTION__)) && g_game().worlds()->getCurrentWorld()->type != WORLD_TYPE_NO_PVP)) {
			return false;
		}
...
bool Player::canWalkthroughEx(std::shared_ptr<Creature> creature) {
	...
	if (player) {
		std::shared_ptr<Tile> playerTile = player->getTile();
		return playerTile && (playerTile->hasFlag(TILESTATE_NOPVPZONE) || playerTile->hasFlag(TILESTATE_PROTECTIONZONE) || player->getLevel() <= static_cast<uint32_t>(g_configManager().getNumber(PROTECTION_LEVEL, __FUNCTION__)) || g_game().worlds()->getCurrentWorld()->type == WORLD_TYPE_NO_PVP);
	} else if (npc) {
		std::shared_ptr<Tile> tile = npc->getTile();
		std::shared_ptr<HouseTile> houseTile = std::dynamic_pointer_cast<HouseTile>(tile);
		return (houseTile != nullptr);
