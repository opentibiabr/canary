	...
	if (damage.primary.value > 0) {
        ...
        if (attackerPlayer) {
            if (!damage.extension && damage.origin != ORIGIN_CONDITION) {
                applyCharmRune(damage, targetMonster, attackerPlayer, target, realDamage);
...
void Game::applyCharmRune(CombatDamage& damage, Monster* targetMonster, Player* attackerPlayer, Creature* target, int32_t& realDamage) const
{
    if(!targetMonster){
        return;
    }
...
double Game::calculateLeechAmount(const int32_t& realDamage, const uint16_t& skillAmount, int targetsAffected) const
{
	return std::round(realDamage * (skillAmount / 100.) * (0.1 * targetsAffected + 0.9) / targetsAffected);
}
