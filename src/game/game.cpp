...
bool Game::combatBlockHit(CombatDamage &damage, const std::shared_ptr<Creature> &attacker, const std::shared_ptr<Creature> &target, bool checkDefense, bool checkArmor, bool field, bool condition /* = false */) {
	...
	// Skill dodge (ruse)
	if (targetPlayer) {
		uint32_t chance = targetPlayer->getDodgeChance();
		if (targetPlayer->isStanceActive(314) && attacker && !Position::areInRange<1, 1, 0>(target->getPosition(), attacker->getPosition())) {
			chance = std::min<uint32_t>(10000, chance + 1200);
		}
...
void Game::applyManaLeech(
	...
	uint16_t manaSkill = attackerPlayer->getSkillLevel(SKILL_MANA_LEECH_AMOUNT) + damage.manaLeech;
...
void Game::applyLifeLeech(
	...
	uint16_t lifeSkill = attackerPlayer->getSkillLevel(SKILL_LIFE_LEECH_AMOUNT) + damage.lifeLeech;
