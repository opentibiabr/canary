...
bool GameStore::isValidType(OfferTypes_t type)
{
...
bool GameStore::loadFromXml(bool /* reloading */) {
	...
	for (auto baseNode : doc.child("store").children()) {
		...
		if (strcasecmp(baseNode.name(), "category") == 0) {
			...
			if (!name) {
				std::cout << "[Warning - GameStore::loadFromXml] Missing name for Category entry" << std::endl;
				continue;
			}
		...
		// Offers
			...
			if (!attributeName) {
				std::cout << "[Warning - GameStore::loadFromXml] Missing name for Offers entry" << std::endl;
				continue;
			}
			...
			if (!res.second) {
				std::cout << "[Warning - GameStore::loadFromXml] Duplicate Category Offer by name: '" << name << "' [ignored]" << std::endl;
				continue;
			}
			...
			for (auto childNode : baseNode.children()) {
				if (strcasecmp(childNode.name(), "offer") == 0) {
					if (!(attr = childNode.attribute("name"))) {
						std::cout << "[Warning - GameStore::loadFromXml] Missing key attribute in GameStore::StoreOffers::StoreOffer name" << std::endl;
						continue;
					}
					...
					if (!resultOffer.second) {
						std::cout << "[Warning - GameStore::loadFromXml] Duplicate Store Offer by name: '" << attr.value() << "'" << std::endl;
						continue;
					}
					...
					if (!childNodeName) {
						std::cout << "[Warning - GameStore::loadFromXml] Store Offer by name: '" << offer.name << "' need price" << std::endl;
						continue;
					}
					...
					if (offer.type == OFFER_TYPE_OUTFIT || offer.type == OFFER_TYPE_OUTFIT_ADDON) {
						...
						if (!childNodeName) {
							std::cout << "[Warning - GameStore::loadFromXml] Store Offer by name: '" << offer.name << "' need female outfit" << std::endl;
							continue;
						}
						offer.female = pugi::cast<uint16_t>(childNodeName.value());
						childNodeName = childNode.attribute("male");
						if (!childNodeName) {
							std::cout << "[Warning - GameStore::loadFromXml] Store Offer by name: '" << offer.name << "' need male outfit" << std::endl;
							continue;
						}
						...
						if (!childNodeName) {
							std::cout << "[Warning - GameStore::loadFromXml] Store Offer by name: '" << offer.name << "' need bless id" << std::endl;
							continue;
						}
						...
						if (!childNodeName) {
							std::cout << "[Warning - GameStore::loadFromXml] Store Offer by name: '" << offer.name << "' need itemtype" << std::endl;
							continue;
						}
						...
						if (!childNodeName) {
							std::cout << "[Warning - GameStore::loadFromXml] Store Offer by name: '" << offer.name << "' need items" << std::endl;
							continue;
						}
...
std::vector<StoreOffers*> GameStore::getStoreOffers()
{
...
std::vector<StoreOffer*> GameStore::getStoreOffer(StoreOffers* offers)
{
...
StoreOffer* GameStore::getStoreOfferByName(std::string name)
{
	for (auto& info : storeOffers) {
		StoreOffers* offers = &info.second;
...
std::vector<StoreOffer*> GameStore::getHomeOffers()
{
	std::vector<StoreOffer*> filter;
...
std::map<std::string, std::vector<StoreOffer*>> GameStore::getHomeOffersOrganized()
{
...
std::map<std::string, std::vector<StoreOffer*>> GameStore::getStoreOrganizedByName(StoreOffers* offers)
{
	std::map<std::string, std::vector<StoreOffer*>> filter;
	for (auto& info : offers->offers) {
		StoreOffer* offer = offers->getOfferByID(info.first);
...
StoreOffers* GameStore::getOfferByName(std::string name)
{
	...
	if (it == storeOffers.end()) {
		...
		// std::cout << "[Warning - GameStore::getOfferByName] Offer " << name << " not found" << std::endl;
		return nullptr;
	}
	return &it->second;
}

StoreOffers* GameStore::getOffersByOfferId(uint32_t id)
{
	for (auto& info : storeOffers) {
		StoreOffers* offers = &info.second;
		if (offers == nullptr){
...
StoreOffer* GameStore::getOfferById(uint32_t id)
{
	for (auto& info : storeOffers) {
		StoreOffers* offers = &info.second;
		if (offers == nullptr){
