...
void Dispatcher::addTask(std::function<void (void)> functor) try {
	...
	asio::post(io_service,
		[this, f = std::move(functor)]() mutable {
...
uint64_t Dispatcher::addEvent(uint32_t delay, std::function<void (void)> functor) try {
