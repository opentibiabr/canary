...
bool FileLoader::openFile(const char* filename, const char* accept_identifier)
{
	...
	char identifier[4];
	if (fread(identifier, 1, 4, getFile()) < 4) {
		fclose(getFile());
		setFile(nullptr);
		lastError = ERROR_EOF;
		return false;
	...
	int32_t file_size = ftell(getFile());
...
int32_t FileLoader::loadCacheBlock(uint32_t pos)
{
	...
	if (fseek(getFile(), cached_data[loading_cache].base, SEEK_SET) != 0) {
		lastError = ERROR_SEEK_ERROR;
		return -1;
	}

	uint32_t size = fread(cached_data[loading_cache].data, 1, cache_size, getFile());
