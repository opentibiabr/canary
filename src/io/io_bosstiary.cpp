...
phmap::parallel_flat_hash_set<uint16_t> IOBosstiary::getBosstiaryFinished(const std::shared_ptr<Player> &player, uint8_t level /* = 1*/) const {
...
std::vector<uint16_t> IOBosstiary::getBosstiaryCooldownRaceId(const std::shared_ptr<Player> &player) const {
