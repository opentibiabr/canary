...
public:
	...
	bool parseCharmCombat(const std::shared_ptr<Charm> charm, const std::shared_ptr<Player> & player, std::shared_ptr<Creature> target, int32_t realDamage, bool dueToPotion = false, bool checkArmor = false);
	...
	void setCharmRuneCreature(const std::shared_ptr<Player> & player, const std::shared_ptr<Charm> charm, uint16_t raceid);
	void resetCharmRuneCreature(const std::shared_ptr<Player> & player, const std::shared_ptr<Charm> charm);
	...
	phmap::parallel_flat_hash_set<uint16_t> getBestiaryFinished(const std::shared_ptr<Player> & player) const;

	charmRune_t getCharmFromTarget(const std::shared_ptr<Player> & player, const std::shared_ptr<MonsterType> mtype);

	std::map<uint16_t, uint32_t> getBestiaryKillCountByMonsterIDs(const std::shared_ptr<Player> & player, std::map<uint16_t, std::string> mtype_list) const;
