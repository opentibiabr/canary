...
bool IOLoginData::loadPlayerById(std::shared_ptr<Player> player, uint32_t id, bool disableIrrelevantInfo /* = true*/) {
	...
	std::string query = fmt::format("SELECT * FROM `players` WHERE `id` = {} AND `worldId` = {}", id, g_gameworld().getWorldId());
...
bool IOLoginData::loadPlayerByName(std::shared_ptr<Player> player, const std::string &name, bool disableIrrelevantInfo /* = true*/) {
	Database &db = Database::getInstance();
	std::string query = fmt::format("SELECT * FROM `players` WHERE `name` = {} AND `worldId` = {}", db.escapeString(name), g_gameworld().getWorldId());
	return loadPlayer(player, db.storeQuery(query), disableIrrelevantInfo);
}
...
std::string IOLoginData::getNameByGuid(uint32_t guid) {
	std::string query = fmt::format("SELECT `name` FROM `players` WHERE `id` = {} AND `worldId` = {}", guid, g_gameworld().getWorldId());
...
uint32_t IOLoginData::getGuidByName(const std::string &name) {
	...
	std::string query = fmt::format("SELECT `id` FROM `players` WHERE `name` = {} AND `worldId` = {}", db.escapeString(name), g_gameworld().getWorldId());
...
bool IOLoginData::getGuidByNameEx(uint32_t &guid, bool &specialVip, std::string &name) {
	...
	std::string query = fmt::format("SELECT `name`, `id`, `group_id`, `account_id` FROM `players` WHERE `name` = {} AND `worldId` = {}", db.escapeString(name), g_gameworld().getWorldId());
	DBResult_ptr result = db.storeQuery(query);
	if (!result) {
		return false;
	}
...
bool IOLoginData::formatPlayerName(std::string &name) {
	...
	std::string query = fmt::format("SELECT `name` FROM `players` WHERE `name` = {} AND `worldId` = {}", db.escapeString(name), g_gameworld().getWorldId());
...
void IOLoginData::increaseBankBalance(uint32_t guid, uint64_t bankBalance) {
	std::string query = fmt::format("UPDATE `players` SET `balance` = `balance` + {} WHERE `id` = {} AND `worldId` = {}", bankBalance, guid, g_gameworld().getWorldId());
	Database::getInstance().executeQuery(query);
}
...
bool IOLoginData::hasBiddedOnHouse(uint32_t guid) {
	...
	std::string query = fmt::format("SELECT `id` FROM `houses` WHERE `highest_bidder` = {} AND `worldId` = {} LIMIT 1", guid, g_gameworld().getWorldId());
...
void IOLoginData::editVIPEntry(uint32_t accountId, uint32_t guid, const std::string &description, uint32_t icon, bool notify) {
	std::string query = fmt::format(
		...
		g_database().escapeString(description), icon, notify, accountId, guid, g_gameworld().getWorldId()
	);
	if (!g_database().executeQuery(query)) {
...
void IOLoginData::removeVIPEntry(uint32_t accountId, uint32_t guid) {
	std::string query = fmt::format("DELETE FROM `account_viplist` WHERE `account_id` = {} AND `player_id` = {} AND `worldId` = {}", accountId, guid, g_gameworld().getWorldId());
