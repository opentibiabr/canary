...
uint32_t MoveEvent::EquipItem(const std::shared_ptr<MoveEvent> moveEvent, std::shared_ptr<Player> player, std::shared_ptr<Item> item, Slots_t slot, bool isCheck) {
	...
	if (it.abilities) {
		if (it.abilities->invisible) {
			std::shared_ptr<Condition> condition = Condition::createCondition(static_cast<ConditionId_t>(slot), ConditionType::Invisible, -1, 0);
			player->addCondition(condition);
		}

		if (it.abilities->manaShield) {
			std::shared_ptr<Condition> condition = Condition::createCondition(static_cast<ConditionId_t>(slot), ConditionType::ManaShield, -1, 0);
			player->addCondition(condition);
		}
		...
		if (it.abilities->regeneration) {
			std::shared_ptr<Condition> condition = Condition::createCondition(static_cast<ConditionId_t>(slot), ConditionType::Regeneration, -1, 0);
...
uint32_t MoveEvent::DeEquipItem(const std::shared_ptr<MoveEvent> MoveEvent, std::shared_ptr<Player> player, std::shared_ptr<Item> item, Slots_t slot, bool) {
	...
	if (it.abilities) {
		if (it.abilities->invisible) {
			player->removeCondition(ConditionType::Invisible, static_cast<ConditionId_t>(slot));
		}

		if (it.abilities->manaShield) {
			player->removeCondition(ConditionType::ManaShield, static_cast<ConditionId_t>(slot));
		}

		if (it.abilities->regeneration) {
			player->removeCondition(ConditionType::Regeneration, static_cast<ConditionId_t>(slot));
		}
