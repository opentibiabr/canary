...
int GlobalFunctions::luaDoAreaCombatHealth(lua_State* L) {
	...
	if (area || areaId == 0) {
		...
		Combat::doCombatHealth(creature, getLuaPosition(L, 3), area, damage, params);
		pushBoolean(L, true);
	} else {
...
int GlobalFunctions::luaDoAreaCombatMana(lua_State* L) {
	...
	if (area || areaId == 0) {
		...
		Position pos = getLuaPosition(L, 2);
...
int GlobalFunctions::luaDoAreaCombatCondition(lua_State* L) {
	...
	if (area || areaId == 0) {
		...
		Combat::doCombatCondition(creature, getLuaPosition(L, 2), area, params);
		pushBoolean(L, true);
	} else {
		reportErrorFunc(getErrorDesc(LUA_ERROR_AREA_NOT_FOUND));
		pushBoolean(L, false);
...
int GlobalFunctions::luaDoAreaCombatDispel(lua_State* L) {
	...
	if (area || areaId == 0) {
		...
		Combat::doCombatDispel(creature, getLuaPosition(L, 2), area, params);
