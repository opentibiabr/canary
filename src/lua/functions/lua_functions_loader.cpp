...
Position LuaFunctionsLoader::getPositionFromLua(lua_State* L, int32_t arg, int32_t& stackpos) {
	Position position;
...
Position LuaFunctionsLoader::getPositionFromLua(lua_State* L, int32_t arg) {
	Position position;
	position.x = getField<uint16_t>(L, arg, "x");
	position.y = getField<uint16_t>(L, arg, "y");
	position.z = getField<uint8_t>(L, arg, "z");
...
LuaVariant LuaFunctionsLoader::getVariant(lua_State* L, int32_t arg) {
	...
	switch (var.type = getField<LuaVariantType_t>(L, arg, "type")) {
		...
		case VARIANT_TARGETPOSITION: {
			...
			var.pos = getPositionFromLua(L, lua_gettop(L));
			lua_pop(L, 2);
			break;
		}
