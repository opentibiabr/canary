...
int PositionFunctions::luaPositionSendMagicEffect(lua_State* L) {
	...
	if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) && !g_game.isMagicEffectRegistered(magicEffect)) {
		SPDLOG_WARN("[PositionFunctions::luaPositionSendMagicEffect] An unregistered magic effect type with id '{}' was blocked to prevent client crash.", effect);
		pushBoolean(L, false);
		return;
	}
...
int PositionFunctions::luaPositionSendDistanceEffect(lua_State* L) {
	...
	if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) && !g_game.isDistanceEffectRegistered(distanceEffect)) {
		SPDLOG_WARN("[PositionFunctions::luaPositionSendDistanceEffect] An unregistered distance effect type with id '{}' was blocked to prevent client crash.", effect);
		return;
	}
