...
int PositionFunctions::luaPositionCreate(lua_State* L) {
	...
	int32_t stackpos;
	if (isTable(L, 2)) {
		const Position& position = getPositionFromLua(L, 2, stackpos);
...
int PositionFunctions::luaPositionAdd(lua_State* L) {
	...
	int32_t stackpos;
	const Position& position = getPositionFromLua(L, 1, stackpos);

	Position positionEx;
	if (stackpos == 0) {
		positionEx = getPositionFromLua(L, 2, stackpos);
	} else {
		positionEx = getPositionFromLua(L, 2);
	}
...
int PositionFunctions::luaPositionCompare(lua_State* L) {
	// position == positionEx
	const Position& positionEx = getPositionFromLua(L, 2);
	const Position& position = getPositionFromLua(L, 1);
...
int PositionFunctions::luaPositionGetPathTo(lua_State* L) {
	...
	const Position& pos = getPositionFromLua(L, 1);
