...
void TileFunctions::init(lua_State* L) {
	// Tile userdata holds a `PolyPtr<Tile>::Shared`; `registerPolyClass`
	// wires the __gc that releases the Shared so the PolyPtr block retires
	// when Lua collects the userdata.
	Lua::registerPolyClass<Tile>(L, "Tile", "", TileFunctions::luaTileCreate);
...
int TileFunctions::luaTileGetPosition(lua_State* L) {
	...
	const auto &tile = Lua::getUserdataPoly<Tile>(L, 1, "Tile");
	if (tile) {
...
int TileFunctions::luaTileGetItemById(lua_State* L) {
	...
	const auto &item = g_game().findItemOfType(tile->getCylinder(), itemId, false, subType);
	if (item) {
		Lua::pushUserdata<Item>(L, item);
		Lua::setItemMetatable(L, -1, item);
	} else {
...
int TileFunctions::luaTileAddItem(lua_State* L) {
	...
	ReturnValue ret = g_game().internalAddItem(tile->getCylinder(), item, INDEX_WHEREEVER, flags);
...
int TileFunctions::luaTileGetHouse(lua_State* L) {
	...
	if (auto* houseTile = dynamic_cast<HouseTile*>(tile)) {
...
int TileFunctions::luaTileSweep(lua_State* L) {
	...
	auto* houseTile = dynamic_cast<HouseTile*>(tile);
	if (!houseTile) {
