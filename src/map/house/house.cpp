		...
		if (const TileItemVector* items = tile->getItemList()) {
			for (auto it = items->rbegin(), end = items->rend(); it != end; ++it) {
				...
				if (item->isWrapable()) {
					...
					Item* newItem = g_game.transformItem(item, Item::items.getItemIdByClientId(ITEM_DECORATION_KIT).id);
...
HouseTransferItem* HouseTransferItem::createHouseTransferItem(House* house)
{
	...
	transferItem->setID(Item::items.getItemIdByClientId(ITEM_DOCUMENT_RO).id);
	...
	for (const auto& it : houseMap) {
		...
		if (player.getBankBalance() >= rent) {
			...
			if (house->getPayRentWarnings() < 7) {
				...
				Item* letter = Item::CreateItem(Item::items.getItemIdByClientId(ITEM_LETTER_STAMPED).id);
				std::string period;
