...
public:
	...
	bool isInList(const std::shared_ptr<Player> & player);
...
public:
	...
	bool canEditAccessList(uint32_t listId, const std::shared_ptr<Player> & player);
	...
	bool isInvited(const std::shared_ptr<Player> & player);

	AccessHouseLevel_t getHouseAccessLevel(const std::shared_ptr<Player> & player);
	...
	bool executeTransfer(std::shared_ptr<HouseTransferItem> item, const std::shared_ptr<Player> & player);
...
private:
	...
	void handleWrapableItem(ItemList &moveItemList, std::shared_ptr<Item> item, const std::shared_ptr<Player> & player, std::shared_ptr<HouseTile> houseTile) const;
};
