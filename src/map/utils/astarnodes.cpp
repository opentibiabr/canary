...
#endif
{
	if (g_cpuinfo().hasAVX2()) {
		const __m256i defaultCost = _mm256_set1_epi32(std::numeric_limits<int32_t>::max());
		...
		const __m128i defaultCost = _mm_set1_epi32(std::numeric_limits<int32_t>::max());
	...
	nodesTable[0] = (x << 16) | y;
	if (g_cpuinfo().hasSSE2() || g_cpuinfo().hasAVX2()) {
		calculatedNodes[0] = 0;
	}
}
