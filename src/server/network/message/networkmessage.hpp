...
public:
	...
	uint16_t getLengthHeader() const {
		// Usar cast explícito para garantir o tipo correto
		return safe_convert<uint16_t>(static_cast<uint16_t>(buffer[0]) | static_cast<uint16_t>(buffer[1]) << 8, __FUNCTION__);
	}
