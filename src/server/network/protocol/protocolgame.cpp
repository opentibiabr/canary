...
void ProtocolGame::AddItem(NetworkMessage &msg, const std::shared_ptr<Item> &item) {
    ...
    // OTCR Features
    if (isOTCR) {
        std::string shader = item->getShader();
        msg.addString(shader.empty() ? "none" : shader); // g_game.enableFeature(GameItemShader)
    }
}
...
void ProtocolGame::sendBestiaryCharms() {
    ...
    for (const auto &c_type : charmList) {
        ...
        // const auto &charmPoints = c_type->points; // Unused variable
