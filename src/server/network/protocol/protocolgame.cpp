...
void ProtocolGame::sendHighscores(const std::vector<HighscoreCharacter> &characters, uint8_t categoryId, uint32_t vocationId, uint16_t page, uint16_t pages, uint32_t updateTimer) {
	...
	msg.addString(g_configManager().getString(SERVER_NAME), "ProtocolGame::sendHighscores - g_configManager().getString(SERVER_NAME)"); // First World
	msg.addString(g_configManager().getString(SERVER_NAME), "ProtocolGame::sendHighscores - g_configManager().getString(SERVER_NAME)"); // Selected World
	...
	for (const HighscoreCharacter &character : characters) {
		...
		msg.addString(g_configManager().getString(SERVER_NAME), "ProtocolGame::sendHighscores - g_configManager().getString(SERVER_NAME)"); // World
...
void ProtocolGame::sendAddCreature(std::shared_ptr<Creature> creature, const Position &pos, int32_t stackpos, bool isLogin) {
	...
	msg.addString(g_configManager().getString(STORE_IMAGES_URL), "ProtocolGame::sendAddCreature - g_configManager().getString(STORE_IMAGES_URL)");
...
void ProtocolGame::addImbuementInfo(NetworkMessage &msg, uint16_t imbuementId) const {
	...
	msg.addString(baseImbuement->name + " " + imbuement->getName(), "ProtocolGame::addImbuementInfo - baseImbuement->name + " " + imbuement->getName()");
...
void ProtocolGame::sendInventoryImbuements(const std::map<Slots_t, std::shared_ptr<Item>> items) {
	...
	for (const auto &[slot, item] : items) {
		...
		for (uint8_t imbueSlot = 0; imbueSlot < slots; imbueSlot++) {
			...
			const BaseImbuement* baseImbuement = g_imbuements().getBaseByID(imbuement->getBaseID());
			msg.addByte(0x01);
			msg.addString(baseImbuement->name + " " + imbuement->getName(), "ProtocolGame::sendInventoryImbuements - baseImbuement->name + " " + imbuement->getName()");
			msg.add<uint16_t>(imbuement->getIconID());
			msg.add<uint32_t>(imbuementInfo.duration);
