	...
	switch (msg.getByte()) {
		...
		case 0xFF: {
			if (msg.getString(4) == "info") {
				g_dispatcher().addTask(createTask(std::bind_front(
		...
		case 0x01: {
			...
			g_dispatcher().addTask(createTask(std::bind_front(&ProtocolStatus::sendInfo, std::static_pointer_cast<ProtocolStatus>(shared_from_this()),
