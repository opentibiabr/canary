...
void ProtocolStatus::sendStatusString() {
	...
	serverinfo.append_attribute("servername") = g_configManager().getString(ConfigKey::SERVER_NAME).c_str();
...
void ProtocolStatus::sendInfo(uint16_t requestedInfo, const std::string &characterName) {
	...
	if (requestedInfo & REQUEST_BASIC_SERVER_INFO) {
		output->addByte(0x10);
		output->addString(g_configManager().getString(ConfigKey::SERVER_NAME));
