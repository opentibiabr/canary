...
class SpectatorVec {
	...
	public:
		...
		inline void emplace_back(Creature* c) {
			vec.emplace_back(c);
		}

		void push_back(Creature* c) {
			vec.push_back(c);
		}
		...
		bool contains(Creature* spectator) const {
			for (const auto &existingSpectator : vec) {
				if (existingSpectator == spectator) {
					return true;
				}
			}
			return false;
