...
int64_t OTSYS_TIME(bool useTime)
{
	...
	int64_t time = (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count()) + (g_configManager().getNumber(TIME_GMT) * 1000);
	return time;
}

int32_t OS_TIME(time_t* timer)
{
	return (time(timer) + g_configManager().getNumber(TIME_GMT));
}
