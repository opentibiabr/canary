...
test("preference unlock has no payload", function()
	local sentBefore = #sentMessages
	local reader = makeReader({ api.action.unlockPreference }, { 1 })
	api.actions.handle(player, reader)
	assert_equal("byte", reader.calls[1])
...
test("preference clear consumes a byte slot", function()
	local reader = makeReader({ api.action.clearPreferred, 0 }, { 1, 1 })
	api.actions.handle(player, reader)
	assert_equal("byte", reader.calls[1])
	assert_equal("byte", reader.calls[2])
	assert_equal(0, reader:getUnreadBytes())
end)

test("preference clear rejects a trailing byte", function()
	local sentBefore = #sentMessages
	local reader = makeReader({ api.action.clearUnwanted, 0 }, { 1, 2 })
	api.actions.handle(player, reader)
	assert_equal(sentBefore, #sentMessages)
end)
